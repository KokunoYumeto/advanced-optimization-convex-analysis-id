...
**Status:** karya dalam proses. Unit Habring Bab 3 (*Subgradien*) dan Bab 4 (*Metode subgradien terproyeksi*) telah diterjemahkan serta lolos audit struktural, matematika, komputasi, pembangunan deterministik, dan pemeriksaan visual. Bab 5 adalah kursor produksi berikutnya. Status ini bukan klaim bahwa keseluruhan korpus telah selesai.
...
Prasyarat yang telah diverifikasi: MiKTeX/pdfTeX, `latexmk`, dan Biber. Dari `source/id-ID`, bangun unit Bab 4 dengan:

```powershell
$env:SOURCE_DATE_EPOCH='1786643315'
$env:TZ='UTC'
latexmk -pdf -interaction=nonstopmode -halt-on-error -file-line-error -outdir='..\..\build\habring-unit-04-id' 'D90-HAB-04-metode-subgradien-terproyeksi-id.tex'
```

Jalankan audit dan pemeriksaan numerik dari akar repositori:

```powershell
python qa/audit_projected_subgradient_unit.py
python qa/validate_projected_subgradient_unit.py
