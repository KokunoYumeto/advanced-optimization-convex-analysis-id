#!/usr/bin/env python3
"""Build and validate the isolated deterministic Becker-02 PDF."""
...
WRAPPER = SOURCE_DIR / "D90-BECKER-02-pemisahan-douglas-rachford-id.tex"
BODY = SOURCE_DIR / "becker-02-pemisahan-douglas-rachford-id.tex"
...
BUILD_ROOT = ROOT / "build" / "becker-02" / "determinism"
OUTPUT = (
    ROOT
    / "output"
    / "pdf"
...
REPORT = ROOT / "qa" / "BECKER_02_PDF_BUILD.json"
JOB = WRAPPER.stem
...
def run_build(path: Path) -> dict[str, object]:
    ...
    if not pdf.is_file() or not log.is_file():
        raise RuntimeError("pdflatex did not create the expected Becker-02 PDF/log")
    ...
    if any(forbidden.values()):
        raise RuntimeError(f"Strict Becker-02 TeX log gate failed: {forbidden}")
    return {
...
def validate_pdf(path: Path) -> dict[str, object]:
    ...
    if reader.is_encrypted:
        raise RuntimeError("Becker-02 PDF is encrypted")
    if not 5 <= len(reader.pages) <= 20:
        raise RuntimeError(f"Unexpected Becker-02 page count: {len(reader.pages)}")
    root = reader.trailer["/Root"]
    if str(root.get("/Lang")) != "id-ID":
        raise RuntimeError("Becker-02 PDF does not declare id-ID")
    ...
    if "Becker 2" not in title or "Douglas-Rachford" not in title:
...
def main() -> None:
    ...
    if missing:
        raise FileNotFoundError("Missing Becker-02 PDF inputs: " + ", ".join(missing))
    ...
    if input_records != [file_record(path) for path in inputs]:
        raise RuntimeError("Becker-02 PDF inputs changed during the two clean builds")
    pdf1 = BUILD_ROOT / "run1" / f"{JOB}.pdf"
    pdf2 = BUILD_ROOT / "run2" / f"{JOB}.pdf"
    if pdf1.read_bytes() != pdf2.read_bytes():
        raise RuntimeError("Two clean fixed-epoch Becker-02 PDFs are not byte-identical")

    validation = validate_pdf(pdf1)
    OUTPUT.parent.mkdir(parents=True, exist_ok=True)
    shutil.copyfile(pdf1, OUTPUT)
    if OUTPUT.read_bytes() != pdf1.read_bytes():
        raise RuntimeError("Canonical Becker-02 PDF copy differs from validated build")
    report = {
        "schema": "o015-becker-02-pdf-build-v1",
        "result": "pass",
        "source_date_epoch": SOURCE_DATE_EPOCH,
