#!/usr/bin/env python3
"""Build and verify the compact Becker-02 Douglas--Rachford release."""
...
ZIP_NAME = "ADVANCED_OPTIMIZATION_CONVEX_ANALYSIS_ID_BECKER_02_2026.08.25.zip"
ZIP_PATH = HERE / ZIP_NAME
RECEIPT_PATH = HERE / "local-verification-becker-02.json"
...
FILES = [
    ("D90-BECKER-02-pemisahan-douglas-rachford-id.pdf", "output/pdf/D90-BECKER-02-pemisahan-douglas-rachford-id.pdf", "primary PDF reader", "application/pdf"),
    ("D90-BECKER-02-pemisahan-douglas-rachford-id.html", "output/html/D90-BECKER-02-pemisahan-douglas-rachford-id.html", "responsive semantic HTML reader", "text/html"),
    ("source/id-ID/becker-02-pemisahan-douglas-rachford-id.tex", "source/id-ID/becker-02-pemisahan-douglas-rachford-id.tex", "Indonesian translated body", "application/x-tex"),
    ("source/id-ID/D90-BECKER-02-pemisahan-douglas-rachford-id.tex", "source/id-ID/D90-BECKER-02-pemisahan-douglas-rachford-id.tex", "standalone Indonesian reader wrapper", "application/x-tex"),
    ...
    ("source/en/becker-02-douglas-rachford-source.tex", "source/en/becker-02-douglas-rachford-source.tex", "exact English source witness", "application/x-tex"),
    ...
    ("qa/BECKER_02_SOURCE_BOUNDARY.json", "qa/BECKER_02_SOURCE_BOUNDARY.json", "source-boundary receipt", "application/json"),
    ("qa/BECKER_02_MATH_VALIDATION.json", "qa/BECKER_02_MATH_VALIDATION.json", "open mathematics validation", "application/json"),
    ("qa/BECKER_02_PDF_BUILD.json", "qa/BECKER_02_PDF_BUILD.json", "deterministic PDF build receipt", "application/json"),
    ("qa/BECKER_02_PDF_VISUAL_QA.json", "qa/BECKER_02_PDF_VISUAL_QA.json", "all-page PDF visual QA", "application/json"),
    ("qa/BECKER_02_HTML_BUILD.json", "qa/BECKER_02_HTML_BUILD.json", "deterministic HTML build receipt", "application/json"),
    ("qa/BECKER_02_HTML_BROWSER_QA.json", "qa/BECKER_02_HTML_BROWSER_QA.json", "desktop/tablet/phone browser QA", "application/json"),
    ("qa/BECKER_02_INDEPENDENT_REREVIEW.json", "qa/BECKER_02_INDEPENDENT_REREVIEW.json", "independent final rereview", "application/json"),
    ("qa/BECKER_02_BACKEND_EXTENSION.json", "qa/BECKER_02_BACKEND_EXTENSION.json", "stable-ID backend extension receipt", "application/json"),
    ("qa/BECKER_02_BACKEND_VALIDATION.json", "qa/BECKER_02_BACKEND_VALIDATION.json", "independent stable-ID validation", "application/json"),
]

AUTHORED = [
    ("README_BECKER_02.md", "reader-first scope, credits, rights, and caveats", "text/markdown"),
...
REPORTS = {
    ...
    "math": "qa/BECKER_02_MATH_VALIDATION.json",
    "pdf": "qa/BECKER_02_PDF_BUILD.json",
    "pdf_visual": "qa/BECKER_02_PDF_VISUAL_QA.json",
    "html": "qa/BECKER_02_HTML_BUILD.json",
    "browser": "qa/BECKER_02_HTML_BROWSER_QA.json",
    "rereview": "qa/BECKER_02_INDEPENDENT_REREVIEW.json",
    "backend": "qa/BECKER_02_BACKEND_EXTENSION.json",
    "backend_validation": "qa/BECKER_02_BACKEND_VALIDATION.json",
}
...
def validate_inputs() -> dict[str, dict]:
    ...
    pdf = PROJECT / "output/pdf/D90-BECKER-02-pemisahan-douglas-rachford-id.pdf"
    html = PROJECT / "output/html/D90-BECKER-02-pemisahan-douglas-rachford-id.html"
    body = PROJECT / "source/id-ID/becker-02-pemisahan-douglas-rachford-id.tex"
    wrapper = PROJECT / "source/id-ID/D90-BECKER-02-pemisahan-douglas-rachford-id.tex"
    witness = PROJECT / "source/en/becker-02-douglas-rachford-source.tex"
    ...
    if reports["boundary"]["authority"]["commit"] != COMMIT:
        raise RuntimeError("Source boundary is not bound to the frozen Becker commit")
    if not reports["boundary"]["combined_witness"]["interior_exact_source_slice_match"]:
        raise RuntimeError("English witness does not match the exact frozen source slice")
    if reports["backend"]["protected_baseline"]["record_count"] != 3320:
        raise RuntimeError("Backend baseline is not the exact 3,320-record Becker-01 state")
    if reports["backend_validation"]["admission"]["final_records"] != 3430:
        raise RuntimeError("Backend final count is not 3,430")
...
def file_record(rel: str, role: str, media: str) -> dict:
    ...
    return {"bytes": path.stat().st_size, "media_type": media, "path": rel, "role": role, "sha256": sha_file(path)}


def safe_member(rel: str) -> bool:
    p = PurePosixPath(rel)
    return bool(rel) and "\\" not in rel and not p.is_absolute() and ".." not in p.parts and not rel.endswith("/")
...
def prepare_package(reports: dict[str, dict]) -> list[str]:
    ...
    manifest = {
        "schema": "o015-becker-02-compact-release-v1",
        "title": "Optimisasi Lanjut dan Analisis Konveks — Modul Becker 2: Pemisahan Douglas–Rachford — Edisi Bahasa Indonesia",
        "release_date": "2026-08-25",
        "status": {"module": "complete at the exact admitted source boundary", "larger_course_edition": "partial"},
        "primary_reader": "D90-BECKER-02-pemisahan-douglas-rachford-id.pdf",
        "reader_order": ["D90-BECKER-02-pemisahan-douglas-rachford-id.pdf", "D90-BECKER-02-pemisahan-douglas-rachford-id.html"],
        "source_authority": {
        ...
        "rights": {
            ...
            "indonesian_translation_corrections_connective_text_and_new_documentation": "CC BY-SA 4.0",
            "component_specific": True,
            "no_blanket_license_claim": True,
        },
        "credits": ["Stephen Becker", "Mitchell Krock", "Bauschke and Combettes", "Lions and Mercier"],
        ...
        "zip_identity_location": "local-verification-becker-02.json outside the ZIP",
        "upstream_contact": False,
    }
    write_json(PACKAGE / "release-manifest-becker-02.json", manifest)
    ordered = [
        ...
        "source/id-ID/D90-BECKER-02-pemisahan-douglas-rachford-id.tex",
        "source/id-ID/becker-02-pemisahan-douglas-rachford-id.tex",
        "source/id-ID/macros-id.tex",
        "source/id-ID/shinybook.cls",
        "source/en/becker-02-douglas-rachford-source.tex",
        "authority/BECKER_AUTHORITY_FREEZE.md",
        "qa/BECKER_02_SOURCE_BOUNDARY.json",
        "qa/BECKER_02_MATH_VALIDATION.json",
        "qa/BECKER_02_PDF_BUILD.json",
        "qa/BECKER_02_PDF_VISUAL_QA.json",
        "qa/BECKER_02_HTML_BUILD.json",
        "qa/BECKER_02_HTML_BROWSER_QA.json",
        "qa/BECKER_02_INDEPENDENT_REREVIEW.json",
        "qa/BECKER_02_BACKEND_EXTENSION.json",
        "qa/BECKER_02_BACKEND_VALIDATION.json",
        "release-manifest-becker-02.json",
    ]
...
def text_policy(members: list[str]) -> None:
    ...
    for rel in ("README_BECKER_02.md", "LICENSE_TRANSLATION_CC_BY-SA-4.0.md", "D90-BECKER-02-pemisahan-douglas-rachford-id.html", "source/id-ID/D90-BECKER-02-pemisahan-douglas-rachford-id.tex"):
...
def build_zip(path: Path, members: list[str]) -> None:
    with zipfile.ZipFile(path, "w", compression=zipfile.ZIP_DEFLATED, compresslevel=9, allowZip64=True) as archive:
        for rel in members:
            ...
            archive.writestr(info, (PACKAGE / PurePosixPath(rel)).read_bytes(), compress_type=zipfile.ZIP_DEFLATED, compresslevel=9)
...
def verify_zip(members: list[str]) -> list[dict]:
    with zipfile.ZipFile(ZIP_PATH, "r") as archive:
        ...
        for info in infos:
            ...
            entries.append({"path": info.filename, "bytes": len(data), "compressed_bytes": info.compress_size, "sha256": sha_bytes(data), "timestamp": "2026-08-25T00:00:00"})
...
def main() -> None:
    ...
    receipt = {
        "schema": "o015-becker-02-local-release-verification-v1",
        "result": "pass",
        ...
        "zip": {"path": str(ZIP_PATH), "bytes": ZIP_PATH.stat().st_size, "sha256": sha_file(ZIP_PATH), "entry_count": len(entries), "entries": entries, "fixed_timestamp": "2026-08-25T00:00:00", "byte_identical_two_builds": True, "compression": "ZIP_DEFLATED level 9", "contains_itself": False},
        "verification": {"crc_test": "pass", "entry_order_matches_explicit_reader_first_inventory": True, "entry_bytes_match_package_tree": True, "sha256sums_verified": True, "unique_entry_names": True, "absolute_entries": 0, "parent_traversal_entries": 0, "backslash_entries": 0, "directory_entries": 0, "forbidden_project_label_occurrences_in_text_members": 0, "credentials_included": False, "network_used": False, "git_used": False},
        "omissions": {"raw_official_archive": True, "full_backend_dataset": True, "build_trees_and_caches": True, "bulk_provenance_dumps": True},
        "model_provenance": MODEL,
    }
