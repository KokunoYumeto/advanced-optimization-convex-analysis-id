#!/usr/bin/env python3
"""Anonymous exact-byte readback for one explicitly named evidence commit."""
...
USER_AGENT = "o015-l08-evidence-anonymous-readback"
...
def main() -> None:
    ...
    commit = args.commit
    if git_text("rev-parse", f"{commit}^{{commit}}") != commit:
        raise RuntimeError("commit argument is not the exact local commit identity")
    ...
    receipt = {
        ...
        "result": "pass",
        "schema": "o015-github-mit-l08-evidence-public-readback-v1",
        "tree": tree,
