#!/usr/bin/env python3
"""Anonymous exact-byte readback for the complete Habring-v1 checkpoint."""
...
USER_AGENT = "o015-habring-v1-anonymous-public-readback"
...
def main() -> None:
    ...
    if not re.fullmatch(r"[0-9a-f]{40}", commit):
        raise RuntimeError("commit must be an exact 40-hex identity")
    if not re.fullmatch(r"[0-9a-f]{40}", parent_expected):
        raise RuntimeError("parent must be an exact 40-hex identity")
    ...
    receipt = {
        ...
        "result": "pass",
        "schema": "o015-github-habring-v1-complete-spine-public-readback-v1",
        "scope": (
            "Complete Bahasa Indonesia translation/readers for the Habring arXiv "
            "2607.11664v1 structured-source spine, Chapters 1-9; the larger O015 "
            "coursebook remains partial and Becker/original closure remains open."
        ),
        "tree": tree,
