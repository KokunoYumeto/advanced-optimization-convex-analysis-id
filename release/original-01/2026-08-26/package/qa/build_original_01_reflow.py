...
def clean_tmp() -> None:
    ...
    if resolved.parent != expected_parent or resolved.name != "original-01-reflow":
...
def source_analysis(wrapper: str, body: str) -> dict[str, object]:
    ...
    if segments != [f"d90.orig.v1.tr01.seg{index:04d}" for index in range(1, 9)]:
        raise RuntimeError(f"Unexpected Original-01 segment inventory: {segments}")
    ...
    for match in re.finditer(
        r"\\begin\{(theorem|lemma|cor|prop|exercise)\}"
        r"(?:\[([^\]]+)\])?(.*?)\\end\{\1\}",
        body,
        flags=re.DOTALL,
    ):
    ...
    equation_labels = re.findall(r"\\label\{(orig01:eq:[^}]+)\}", body)
    for number, label in enumerate(equation_labels, 1):
    ...
    if body.count(r"\textbf{Petunjuk bertahap.}") != 6:
        raise RuntimeError("Expected six staged hints")
    if body.count(r"\textbf{Solusi lengkap.}") != 6:
        raise RuntimeError("Expected six complete solutions")
    if "konfigurasi\n    beku yang dicatat pada berkas hasil" not in body:
        raise RuntimeError("The repaired laboratory task wording is absent from the live source")
...
def csv_table_xhtml() -> str:
    ...
    if len(rows) != 39 or len(rows[0]) != 8:
    ...
    return (
        ...
        '<caption>Seluruh 38 baris hasil laboratorium dengan biaya oracle tetap</caption>'
...
def validate_common_reader(document: str, analysis: dict[str, object], *, epub: bool) -> dict[str, object]:
    ...
    if re.search(r">\s*\[orig01:[^<]+\]\s*<", document):
    ...
    if math_count != analysis["math_surfaces"]:
        failures.append(
            f"math surface count differs: source={analysis['math_surfaces']}, output={math_count}"
        )
...
def stable_inputs() -> list[dict[str, object]]:
    paths = [WRAPPER, BODY, *LAB_FILES, SCRIPT]
...
def main() -> None:
    ...
    html_report = {
        "schema": "o015-original-01-html-build-v1",
        **shared,
    ...
    epub_report = {
        "schema": "o015-original-01-epub-build-v1",
        **shared,
