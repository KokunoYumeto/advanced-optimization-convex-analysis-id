#!/usr/bin/env python3
"""Fail-closed Zenodo publisher for the consolidated MIT-L04/L05 version.

This module performs no network operation on import.  A later explicit CLI
action is required; ``publish`` validates, publishes, and completes anonymous
public-byte readback in one transaction.
"""
...
READBACK_PATH = HERE / "zenodo-public-readback-mit-l04-l05.json"
EXPECTED_TITLE = "Optimisasi Lanjut dan Analisis Konveks - Edisi Bahasa Indonesia (id-ID): Checkpoint MIT 6.253 Halaman 15-19 (Belum Lengkap)"
...
def session(authenticated: bool = True) -> requests.Session:
    ...
    client.headers["User-Agent"] = "O015-id-ID-MIT-L04-L05-preservation/1.0"
    if authenticated:
...
def save_state(record: dict) -> dict:
    receipt = {
        "schema": "o015-zenodo-mit-l04-l05-draft-receipt-v1",
        ...
        # Keep the intended release identity durable even during the short
        # interval between new-version creation and metadata replacement.
...
def state() -> dict:
    if not STATE_PATH.is_file():
        raise RuntimeError("No local MIT-L04/L05 draft receipt; run prepare first")
    data = json.loads(STATE_PATH.read_text(encoding="utf-8"))
    if (
        data.get("schema") != "o015-zenodo-mit-l04-l05-draft-receipt-v1"
...
def get_draft(client: requests.Session, record_id: str | None = None) -> dict:
    ...
    if parent_id is not None and str(parent_id) != build.CONCEPT_ID:
...
def validate_metadata_shape(metadata: dict) -> None:
    ...
    if (
        ...
        or metadata.get("version") != build.VERSION
        or metadata.get("publication_date") != "2026-08-23"
    ):
        raise RuntimeError("MIT-L04/L05 title/version/date metadata mismatch")
...
def ensure_inherited_imported(client: requests.Session, record_id: str) -> dict[str, dict]:
    ...
    for _ in range(90):
        ...
        if inherited.issubset(set(entries)):
            ...
            return entries
        time.sleep(1)
    raise RuntimeError("files-import did not expose all 40 inherited files")
...
def verify_parent_public() -> dict:
    ...
    if set(entries) != set(expected):
        raise RuntimeError("parent inventory differs from the frozen 40-file readback")
...
def bind_manifest() -> None:
    ...
    lane_closure = build.validate_lane_closure()
    build.build_release_metadata(verification, lane_closure)
...
def prepare() -> dict:
    ...
    if STATE_PATH.is_file():
        ...
        updated = client.put(f"{API}/records/{record_id}/draft", json=template, timeout=60)
...
def upload() -> dict:
    ...
    # This is the only upload loop: inherited files arrive solely through
    # files-import and only the ten collision-free additions are uploaded.
...
def readback(wait_for_publication: bool = False) -> dict:
    ...
    if set(entries) != set(expected) or len(entries) != build.EXPECTED_RELEASE_COUNT:
        raise RuntimeError("anonymous public 50-file inventory mismatch")
    verified = verify_expected_bytes(client, entries, expected)
    inherited_names = set(build.inherited_inventory())
    for item in verified:
        item["disposition"] = (
            "inherited_unchanged" if item["filename"] in inherited_names else "mit_l04_l05_addition"
        )
    bundle_payload = download_entry(client, entries[build.BUNDLE_NAME])
    metadata_serialized = json.dumps(record["metadata"], ensure_ascii=False)
    receipt = {
        "schema": "o015-zenodo-mit-l04-l05-public-readback-v1",
        "record_id": draft_id(),
        ...
        "ttp_metadata_mentions": metadata_serialized.count("TTP"),
        "model_provenance_mentions": metadata_serialized.count(build.MODEL_ID),
        "files": verified,
        "file_count": len(verified),
        "inherited_file_count": sum(item["disposition"] == "inherited_unchanged" for item in verified),
        "addition_file_count": sum(item["disposition"] == "mit_l04_l05_addition" for item in verified),
