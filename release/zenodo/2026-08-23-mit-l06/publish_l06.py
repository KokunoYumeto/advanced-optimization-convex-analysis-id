#!/usr/bin/env python3
"""Fail-closed Zenodo publisher and anonymous readback for MIT L06.

Importing this module performs no network or credential operation.  Every
mutating action first runs the local deterministic release gate.  ``publish``
cannot stop at a public-state claim: it immediately performs anonymous
inventory and full-byte SHA-256 readback.
"""
...
READBACK_PATH = HERE / "zenodo-public-readback-mit-l06.json"
EXPECTED_TITLE = "Optimisasi Lanjut dan Analisis Konveks - Edisi Bahasa Indonesia (id-ID): Checkpoint MIT 6.253 Kuliah 2, Halaman 20-28 (Belum Lengkap)"
...
def session(authenticated: bool = True) -> requests.Session:
    ...
    client.headers["User-Agent"] = "O015-id-ID-MIT-L06-preservation/1.0"
    if authenticated:
...
def save_state(record: dict) -> dict:
    receipt = {
        "schema": "o015-zenodo-mit-l06-draft-receipt-v1",
...
def state() -> dict:
    if not STATE_PATH.is_file():
        raise RuntimeError("No local MIT-L06 draft receipt; run prepare first")
    data = json.loads(STATE_PATH.read_text(encoding="utf-8"))
    if (
        data.get("schema") != "o015-zenodo-mit-l06-draft-receipt-v1"
...
def get_draft(client: requests.Session, record_id: str | None = None) -> dict:
    ...
    if str(parent_id) != build.CONCEPT_ID:
...
def validate_metadata_shape(metadata: dict) -> None:
    ...
    if title != EXPECTED_TITLE or metadata.get("version") != build.VERSION or metadata.get("publication_date") != "2026-08-23":
        raise RuntimeError("MIT-L06 title/version/date metadata mismatch")
    ...
    for required in ("halaman 20-28", "halaman 29", "belum lengkap", "50 berkas induk", "cc by-nc-sa 4.0", "athena"):
...
def validate_record_shape(record: dict, require_preview: bool = True) -> None:
    ...
    if require_preview and record.get("files", {}).get("default_preview") != build.READER_PATHS[0].name:
        raise RuntimeError("remote record does not make the L06 PDF its default preview")
...
def template_payload(include_preview: bool) -> dict:
    ...
    if payload.get("files", {}).get("default_preview") != build.READER_PATHS[0].name:
        raise RuntimeError("local metadata template lacks the required L06 PDF preview")
    ...
    for attempt in range(1, 5):
        try:
            ...
            if attempt < 4:
                ...
                continue
            raise
    raise RuntimeError(f"download failed for {item.get('key')}: {last_error}")
...
def ensure_inherited_imported(client: requests.Session, record_id: str) -> dict[str, dict]:
    ...
    if inherited.issubset(set(entries)):
        if set(entries) - inherited - additions:
            raise RuntimeError("draft contains an unexpected filename before inherited-file verification")
        return entries
    if entries:
    ...
    for _ in range(90):
        ...
        if inherited.issubset(set(entries)):
            ...
            return entries
        time.sleep(1)
    raise RuntimeError("files-import did not expose all 50 inherited files")
...
def verify_parent_public() -> dict:
    ...
    if versions.get("is_latest") is not True or not latest_link:
        raise RuntimeError("required parent is not proved to be the concept's latest public version")
    ...
    if str(latest.get("id")) != build.PARENT_RECORD_ID or latest.get("status") != "published":
        raise RuntimeError("concept latest-version resolution does not equal required parent 22072071")
    if str(latest.get("parent", {}).get("id")) != build.CONCEPT_ID:
        raise RuntimeError("resolved latest parent belongs to a different concept")
    entries = public_entries(record)
    expected = build.inherited_inventory()
    if set(entries) != set(expected):
        raise RuntimeError("parent inventory differs from the frozen 50-file readback")
...
def prepare() -> dict:
    ...
    if STATE_PATH.is_file():
        ...
        if receipt.get("status") == "published":
            raise RuntimeError("this version is already published; refusing to create or reuse another draft")
    ...
    if response.status_code == 409:
        raise RuntimeError("Zenodo reports an existing new-version draft; refusing to create a second draft without its local receipt")
    response.raise_for_status()
...
def upload() -> dict:
    ...
    additions = build.addition_paths()
    order = {build.READER_PATHS[0].name: 0, build.READER_PATHS[1].name: 1}
    for path in sorted(additions, key=lambda item: (order.get(item.name, 2), item.name)):
...
def readback(wait_for_publication: bool = False) -> dict:
    ...
    receipt = {
        ...
        "addition_file_count": sum(item["disposition"] == "mit_l06_addition" for item in verified),
        "inherited_identity": "pass",
        "delta_bundle_verification": build.verify_bundle(bundle_payload),
    }
    if (
...
def reconcile_ambiguous_publish(record_id: str) -> dict | None:
    """Recover when the publish POST outcome is unknown after timeout/5xx."""
    client = session(authenticated=False)
    for attempt in range(1, 5):
        ...
        if response.status_code == 200:
            ...
            if candidate.get("status") == "published":
                ...
                return candidate
        elif response.status_code not in (404, 429) and response.status_code < 500:
            response.raise_for_status()
        if attempt < 4:
            time.sleep(attempt * 2)
    return None
...
def publish() -> dict:
    ...
    try:
        ...
        recovered = reconcile_ambiguous_publish(record_id)
