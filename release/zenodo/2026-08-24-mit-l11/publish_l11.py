#!/usr/bin/env python3
"""Fail-closed Zenodo publisher and anonymous readback for MIT L11.
...
followed by anonymous exact-byte readback of all 98 files.
"""
...
READBACK_PATH = HERE / "zenodo-public-readback-mit-l11.json"
EXPECTED_TITLE = "Optimisasi Lanjut dan Analisis Konveks - Edisi Bahasa Indonesia (id-ID): Checkpoint MIT 6.253 Kuliah 7, Halaman 86-97 (Belum Lengkap)"
...
def session(authenticated: bool = True) -> requests.Session:
    ...
    client.headers["User-Agent"] = "O015-id-ID-MIT-L11-preservation/1.0"
    if authenticated:
...
def save_state(record: dict) -> dict:
    receipt = {
        "schema": "o015-zenodo-mit-l11-draft-receipt-v1",
...
def state() -> dict:
    if not STATE_PATH.is_file():
        raise RuntimeError("No local MIT-L11 draft receipt; run prepare first")
...
def validate_metadata_shape(metadata: dict) -> None:
    ...
    if title != EXPECTED_TITLE or metadata.get("version") != build.VERSION or metadata.get("publication_date") != "2026-08-24":
        raise RuntimeError("MIT-L11 title/version/date metadata mismatch")
    if (
        "TTP" in title
...
def validate_record_shape(record: dict, require_preview: bool = True) -> None:
    ...
    if require_preview and record.get("files", {}).get("default_preview") != build.READER_PATHS[0].name:
        raise RuntimeError("remote record does not make the L11 PDF its default preview")
...
def template_payload(include_preview: bool) -> dict:
    ...
    if payload.get("files", {}).get("default_preview") != build.READER_PATHS[0].name:
        raise RuntimeError("local metadata template lacks the required L11 PDF preview")
...
def verify_parent_public() -> dict:
    ...
    if str(latest.get("id")) != build.PARENT_RECORD_ID or latest.get("status") != "published":
        raise RuntimeError("concept latest-version resolution does not equal required parent 22077419")
    ...
    if set(entries) != set(expected):
        raise RuntimeError("parent inventory differs from the frozen 90-file readback")
...
def ensure_inherited_imported(client: requests.Session, record_id: str) -> dict[str, dict]:
    ...
    for _ in range(90):
        ...
        if inherited.issubset(set(entries)):
            ...
            return entries
        time.sleep(1)
    raise RuntimeError("files-import did not expose all 90 inherited files")
...
def readback(wait_for_publication: bool = False) -> dict:
    ...
    if set(entries) != set(expected) or len(entries) != build.EXPECTED_RELEASE_COUNT:
        raise RuntimeError("anonymous public 98-file inventory mismatch")
    verified = verify_expected_bytes(client, entries, expected)
    inherited_names = set(build.inherited_inventory())
    for item in verified:
        item["disposition"] = "inherited_unchanged" if item["filename"] in inherited_names else "mit_l11_addition"
    serialized = json.dumps(record["metadata"], ensure_ascii=False)
    receipt = {
        "schema": "o015-zenodo-mit-l11-public-readback-v1",
        "result": "pass",
        "record_id": draft_id(),
        ...
        "addition_file_count": sum(item["disposition"] == "mit_l11_addition" for item in verified),
