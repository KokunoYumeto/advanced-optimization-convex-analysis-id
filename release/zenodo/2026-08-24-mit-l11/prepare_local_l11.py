#!/usr/bin/env python3
"""Run the two-build local MIT-L11 release gate and write its receipt.
...
RECEIPT_PATH = HERE / "local-preparation-mit-l11.json"
...
def main() -> None:
    ...
    if not build.INPUT_LOCK_PATH.is_file() or build.INPUT_LOCK_PATH.read_bytes() != expected_lock:
        raise RuntimeError("frozen L11 input lock is absent or stale; regenerate it explicitly first")
    ...
    receipt = {
        "schema": "o015-zenodo-mit-l11-local-preparation-v1",
        ...
        "final_backend": {
            ...
            "validation": identity(build.ROOT / "qa/MIT_L11_BACKEND_VALIDATION.json"),
            "correction_snapshot": identity(build.ROOT / "00_control/MIT_L11_CORRECTION_SNAPSHOT.jsonl"),
        },
        "prepublication_control_bindings": first["lane_closure"]["control_bindings"],
        "release_builder": identity(HERE / "build_l11.py"),
        "input_lock_generator": identity(HERE / "generate_input_lock_l11.py"),
        ...
        "publisher": identity(HERE / "publish_l11.py"),
        "network_operations": 0,
        "credential_reads": 0,
        "git_operations": 0,
        "publication_operations": 0,
        "draft_state_file_exists": build.STATE_PATH.is_file(),
        "public_readback_file_exists": (HERE / "zenodo-public-readback-mit-l11.json").is_file(),
        "note": "Local deterministic preservation package only. A future publisher must create one new version from record 22077419 in concept 10.5281/zenodo.22059741, inherit all 90 parent files byte-for-byte, bind the manifest to that record, publish, and anonymously read back all 98 public files.",
    }
