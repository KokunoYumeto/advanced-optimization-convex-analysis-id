#!/usr/bin/env python3
"""Build and verify the deterministic additive MIT-L11 preservation delta.

The builder is local and performs no network, credential, Git, publication, or
mutable-control operation.  It inherits exactly the 90 files proved by the L10
public readback and adds exactly eight collision-free L11 filenames.  It fails
before writing a bundle until ``release-config-mit-l11.json`` supplies final
...
import generate_input_lock_l11 as lockgen


HERE = Path(__file__).resolve().parent
ROOT = HERE.parents[2]
PREVIOUS_READBACK = HERE.parent / "2026-08-24-mit-l10" / "zenodo-public-readback-mit-l10.json"
TEMPLATE_PATH = HERE / "zenodo-record-mit-l11.json"
INPUT_LOCK_PATH = HERE / "release-input-lock-mit-l11.json"
STATE_PATH = HERE / "zenodo-draft-mit-l11.json"
FIXED_ZIP_TIME = (2026, 8, 24, 0, 0, 0)

PARENT_RECORD_ID = "22077419"
PARENT_RECORD_DOI = "10.5281/zenodo.22077419"
CONCEPT_ID = "22059741"
...
STATUS = "partial"
CORPUS_ROLE = "separately licensed MIT 6.253 companion within O015; not the final canonical editable-source spine"
...
BUNDLE_NAME = "ADVANCED_OPTIMIZATION_CONVEX_ANALYSIS_ID_SOURCE_BACKEND_2026.08.24_MIT_L11_DELTA.zip"
MANIFEST_NAME = "release-manifest-mit-l11.json"
SUMS_NAME = "SHA256SUMS-mit-l11"
EXPECTED_INHERITED_COUNT = 90
EXPECTED_ADDITION_COUNT = 8
EXPECTED_RELEASE_COUNT = 98

READER_PATHS = [
    ROOT / "output/pdf/D90-MIT-11-kuliah-7-pemisahan-dan-konjugasi-id.pdf",
    ROOT / "output/html/D90-MIT-11-kuliah-7-pemisahan-dan-konjugasi-id.html",
]
RELEASE_DOCS = ["README_MIT_L11.md", "README_RELEASE_MIT_L11.md", "LICENSE_MIT_L11.md"]
...
def controls_may_have_advanced() -> bool:
    ...
    if state.get("status") != "published":
        return False
    if (
        state.get("schema") != "o015-zenodo-mit-l11-draft-receipt-v1"
...
def inherited_inventory() -> dict[str, dict[str, object]]:
    ...
    if (
        ...
        raise RuntimeError("frozen readback does not identify the published L10 parent")
    ...
    if (
        ...
        raise RuntimeError("L10 readback does not prove exactly 90 unique public parent files")
    return result
...
def validate_template() -> dict:
    ...
    if template.get("files", {}).get("default_preview") != READER_PATHS[0].name:
        raise RuntimeError("metadata template must make the L11 PDF the default preview")
    ...
    for required in (
        ...
        "cc by-nc-sa 4.0",
        "7 blok relasi gambar",
        "o015-mit-sem-0040",
        "pendamping matematis",
        "arxiv:2607.11664v1",
    ):
...
def validate_input_lock() -> dict:
    ...
    if (
        lock.get("schema") != "o015-mit-l11-release-input-lock-v1"
        ...
        raise RuntimeError("L11 input lock has a different boundary, count, or backend transition")
    files = lock.get("files", {})
    if not isinstance(files, dict) or set(files) != expected_paths:
        raise RuntimeError("L11 input lock does not bind the exact material/control closure")
    ...
    if cfg["backend"]["validation"] != files["qa/MIT_L11_BACKEND_VALIDATION.json"]:
...
def validate_lane_closure() -> dict[str, object]:
    ...
    browser = read_json(ROOT / "qa/MIT_L11_BROWSER_QA.json")
    visual = read_json(ROOT / "qa/MIT_L11_VISUAL_QA.json")
    backend = read_json(ROOT / "qa/MIT_L11_BACKEND_VALIDATION.json")
    rereview_path = ROOT / "qa/MIT_L11_INDEPENDENT_REREVIEW.md"
    ...
    if (
        ...
        or content.get("stage") != "strict-final"
        ...
        or content.get("pdf", {}).get("searchable_text_chars", 0) <= 0
        ...
        or content.get("rights", {}).get("athena_source_figure_panels_omitted") != 16
    ):
        raise RuntimeError("final L11 content validation closure failed")
    ...
    if (
        ...
        or canonical.get("html", {}).get("bytes") != files[html_relative]["bytes"]
        or canonical.get("html", {}).get("sha256") != files[html_relative]["sha256"]
        or canonical.get("pdf", {}).get("bytes") != files[pdf_relative]["bytes"]
        or canonical.get("pdf", {}).get("sha256") != files[pdf_relative]["sha256"]
        or content.get("build", {}).get("deterministic_rebuilds") != 2
        or content.get("formula_inventory", {}).get("witness", {}).get("display_blocks") != 21
        or content.get("formula_inventory", {}).get("target", {}).get("display_blocks") != 21
    ):
        raise RuntimeError("L11 deterministic reader/formula closure failed")
    ...
    semantics = browser.get("semantics", {})
    if (
        browser.get("schema") != "o015-mit-l11-browser-qa-v1"
        ...
        or browser.get("desktop", {}).get("visual_inspection") != "pass"
        or browser.get("mobile", {}).get("visual_inspection") != "pass"
        or semantics.get("source_pages") != 12
        or semantics.get("source_items") != 36
        or semantics.get("source_figures") != 7
        or semantics.get("source_displays") != 21
        or semantics.get("edition_corrections") != 10
        or semantics.get("images") != 0
        or semantics.get("interactive_controls_or_embeds") != 0
        or semantics.get("duplicate_ids") != []
        or semantics.get("unresolved_fragments") != []
        or browser.get("console_findings") != []
    ):
        raise RuntimeError("final L11 browser validation closure failed")

    rendered = visual.get("render", {}).get("pages", [])
    inspection = visual.get("inspection", {})
    structure = visual.get("pdf_structure", {})
    if (
        visual.get("schema") != "o015-mit-l11-visual-qa-v1"
        ...
        or len(rendered) != 6
        or [item.get("page") for item in rendered] != list(range(1, 7))
        or any(not isinstance(item.get("bytes"), int) or item["bytes"] <= 0 or not re.fullmatch(r"[0-9a-f]{64}", item.get("sha256", "")) for item in rendered)
        or inspection.get("all_pages_inspected_at_original_render_resolution") is not True
        or inspection.get("reader_pages") != 6
        or inspection.get("page_size") != "A4"
        or any(inspection.get(key) != 0 for key in ("clipping", "overlap", "truncated_text", "orphaned_outline_items", "isolated_proof_conclusions", "broken_glyphs", "unreadable_formulas", "source_graphic_bytes"))
        or structure.get("lang") != "id-ID"
        or structure.get("tagged") is not False
        or structure.get("encrypted") is not False
        or structure.get("image_xobjects") != 0
    ):
        raise RuntimeError("final L11 visual validation closure failed")

    witness_relative = "source/en/mit-11-lecture-7-separation-conjugacy-semantic-witness.md"
    target_relative = "source/id-ID/mit-11-kuliah-7-pemisahan-dan-konjugasi-id.md"
    if (
        "P1: 0" not in rereview
        or "P2: 0" not in rereview
        or "P3: 0" not in rereview
        or "Independent rereview result: PASS" not in rereview
        or files[witness_relative]["sha256"] not in rereview
        or files[target_relative]["sha256"] not in rereview
    ):
        raise RuntimeError("independent rereview does not bind a clean L11 disposition")

    expected_corrections = lockgen.EXPECTED_EVENTS
    ...
    if (
        ...
        or admission.get("final_records") != final
        or admission.get("new_records") != new
        ...
        raise RuntimeError("final L11 backend validation closure failed")
    for kind, relative in (("jsonl", "backend/records.jsonl"), ("csv", "backend/records.csv")):
        observed = admission.get(kind, {})
    ...
    content_events = [item.get("event_id") for item in content.get("correction_bindings", [])]
    if content_events != expected_corrections:
        raise RuntimeError("content and backend correction-event inventories differ")

    controls = {relative: files[relative] for relative in sorted(lockgen.CONTROL_PATHS)}
    return {
        "result": "pass",
        "source_pages": SOURCE_PAGES,
        "next_source_page": NEXT_SOURCE_PAGE,
        "content_validation": identity(ROOT / "qa/MIT_L11_VALIDATION.json"),
        "browser_validation": identity(ROOT / "qa/MIT_L11_BROWSER_QA.json"),
        "visual_validation": identity(ROOT / "qa/MIT_L11_VISUAL_QA.json"),
        "independent_rereview": identity(rereview_path),
        "backend_validation": identity(ROOT / "qa/MIT_L11_BACKEND_VALIDATION.json"),
        "backend": {
            "record_count": final,
...
def bundle_inputs() -> list[tuple[str, Path]]:
    ...
    pairs.append(("release-notes/release-input-lock-mit-l11.json", INPUT_LOCK_PATH))
...
def verify_bundle(payload: bytes) -> dict[str, object]:
    ...
    with zipfile.ZipFile(io.BytesIO(payload), "r") as archive:
        ...
        if (
            ...
            or manifest.get("corpus_role") != CORPUS_ROLE
            or manifest.get("canonical_editable_spine") != CANONICAL_EDITABLE_SPINE
            or manifest.get("new_habring_bytes") != 0
...
def build_bundle(lane_closure: dict[str, object]) -> tuple[Path, dict[str, object]]:
    ...
    inner = {
        "schema": "o015-mit-l11-delta-bundle-v1",
        "version": VERSION,
        "scope": "MIT 6.253 complete-notes Lecture 7, pages 86-97; compact semantic source, build, QA, backend, boundary evidence, and license closure",
        "status": STATUS,
        ...
        "canonical_editable_spine": CANONICAL_EDITABLE_SPINE,
        "new_habring_bytes": 0,
    ...
    with tempfile.NamedTemporaryFile(prefix=".mit-l11-delta-", suffix=".zip", dir=HERE, delete=False) as handle:
...
def draft_identity() -> tuple[str | None, str | None]:
    ...
    if (
        data.get("schema") != "o015-zenodo-mit-l11-draft-receipt-v1"
        or data.get("parent_record_id") != PARENT_RECORD_ID
        or data.get("concept_id") != CONCEPT_ID
        or data.get("version") != VERSION
    ):
        raise RuntimeError("local draft receipt belongs to a different L11 lineage/version")
...
def build_release_metadata(bundle_verification: dict[str, object], lane_closure: dict[str, object]) -> None:
    ...
    manifest = {
        ...
        "admitted_boundary": "MIT 6.253 complete-notes Lecture 7, PDF pages 86-97, L11",
        "next_boundary": "MIT 6.253 complete-notes PDF page 98, Lecture 8",
        "backend_record_count": final,
        "protected_backend_record_count": protected,
        "new_backend_record_count": new,
        "component_rights": COMPONENT_RIGHTS,
...
def validate_local_release(require_draft_binding: bool = False) -> dict[str, object]:
    ...
    if (
        ...
        or manifest.get("next_boundary") != "MIT 6.253 complete-notes PDF page 98, Lecture 8"
        or manifest.get("component_rights") != COMPONENT_RIGHTS
        or manifest.get("corpus_role") != CORPUS_ROLE
        or manifest.get("canonical_editable_spine") != CANONICAL_EDITABLE_SPINE
        or manifest.get("new_habring_bytes") != 0
    ...
    if require_draft_binding:
        ...
        if manifest.get("zenodo_record_id") != record_id or manifest.get("zenodo_record_doi") != record_doi:
            raise RuntimeError("release manifest is not bound to the prepared L11 draft")
