#!/usr/bin/env python3
"""Build, publish, and anonymously verify the Becker-03 Zenodo checkpoint.
...
PARENT_RECORD_ID = "22098168"
PARENT_RECORD_DOI = "10.5281/zenodo.22098168"
PARENT_VERSION = "checkpoint-2026.08.25-becker-02-douglas-rachford"
PARENT_TITLE = (
    "Optimisasi Lanjut dan Analisis Konveks — Edisi Bahasa Indonesia (id-ID): "
    "Pemisahan Douglas–Rachford; Kursus Parsial"
)
CONCEPT_ID = "22059741"
CONCEPT_DOI = "10.5281/zenodo.22059741"
VERSION = "checkpoint-2026.08.25-becker-03-variance-reduction"
MODEL_ID = "OpenAI Codex gpt-5.6-sol, Ultra"
FORBIDDEN_ORG_EXPANSION = "Translation and Transcription Project"
GITHUB_COMMIT = "09cc7e554b87c735428fb8cf3320a3d499956894"
GITHUB_TREE = "babb37fc16f31f95fee3e34a9b941d23e25a564b"
PRIMARY_PDF = "D90-BECKER-03-reduksi-varians-id.pdf"

PARENT_READBACK = (
    ROOT
    / "release"
    / "zenodo"
    / "2026-08-25-becker-02"
    / "zenodo-public-readback-becker-02.json"
)
STATE_PATH = HERE / "zenodo-draft-becker-03.json"
READBACK_PATH = HERE / "zenodo-public-readback-becker-03.json"
DRAFT_CLOSURE_PATH = HERE / "zenodo-draft-closure-becker-03.json"
PREVIEW_REPAIR_PATH = HERE / "zenodo-default-preview-repair-becker-03.json"
MANIFEST_PATH = HERE / "release-manifest-becker-03-zenodo.json"
SUMS_PATH = HERE / "SHA256SUMS-becker-03"
RIGHTS_PATH = HERE / "RIGHTS_AND_PROVENANCE_BECKER_03.md"

CORE_PUBLIC_PATHS = {
    "D90-BECKER-03-reduksi-varians-id.pdf": ROOT
    / "output/pdf/D90-BECKER-03-reduksi-varians-id.pdf",
    "D90-BECKER-03-reduksi-varians-id.html": ROOT
    / "output/html/D90-BECKER-03-reduksi-varians-id.html",
    "ADVANCED_OPTIMIZATION_CONVEX_ANALYSIS_ID_BECKER_03_2026.08.25.zip": ROOT
    / "release/becker-03/2026-08-25/ADVANCED_OPTIMIZATION_CONVEX_ANALYSIS_ID_BECKER_03_2026.08.25.zip",
    "backend-records-2026.08.25-becker-03.jsonl": ROOT / "backend/records.jsonl",
    "backend-records-2026.08.25-becker-03.csv": ROOT / "backend/records.csv",
...
REPLACED_PARENT_FILES = {
    ...
    "ADVANCED_OPTIMIZATION_CONVEX_ANALYSIS_ID_BECKER_01_2026.08.25.zip",
    "RIGHTS_AND_PROVENANCE_BECKER_01.md",
    "SHA256SUMS-becker-01",
    "release-manifest-becker-01-zenodo.json",
    "SHA256SUMS-becker-02",
}
...
def parent_inventory() -> dict[str, dict[str, object]]:
    ...
    if len(entries) != 99 or not REPLACED_PARENT_FILES.issubset(entries):
...
def metadata_payload() -> dict:
    description = (
        ...
        "Kursus yang lebih besar tetap parsial. Versi ini menambahkan modul nonduplikatif reduksi varians untuk "
        "aproksimasi rata-rata sampel dari repositori Stephen Becker pada commit 98ed6930084c435ba0f675f7646ced1f2fd8729e, "
        ...
        "<p>Modul mempertahankan tepat baris 2971–2988 dan mengecualikan materi stokastik sebelumnya serta terminator "
        "dokumen. Lima koreksi matematis dan editorial diumumkan. Dua build PDF/HTML deterministik, sebelas uji "
        "matematika terbuka, reflow desktop/tablet/seluler, inspeksi semua sebelas halaman PDF, rereview independen, "
        "dan backend lulus. Backend mempertahankan 3.430 rekaman sebelumnya dan menambahkan 155 rekaman, menjadi 3.585. "
        "Dua snapshot backend Becker-02 dan lima alat bantu rilis lama yang redundan tidak diulang hanya pada versi baru ini, "
        "agar PDF/HTML Becker-03, paket kelanjutan ringkas, backend mutakhir, catatan hak, manifest, dan checksum tetap berada "
        "dalam batas 100 berkas Zenodo; semua berkas yang tidak diulang tetap tak berubah dan tersedia pada versi Zenodo terdahulu.</p>"
        "<p>Hak berlaku per komponen. Sumber Becker berlisensi MIT dan dikreditkan kepada Stephen Becker serta Mitchell "
        "Krock. Hasil matematis SAGA dikreditkan secara terpisah kepada Aaron Defazio, Francis Bach, dan Simon "
        "Lacoste-Julien; makalah mereka hanya menjadi saksi hasil, PDF-nya tidak didistribusikan, dan tidak ada prosa atau "
        "tata letaknya yang disalin. Terjemahan, kata-kata koreksi, bukti penghubung, latihan, petunjuk, dan solusi "
        "independen berlisensi CC BY-SA 4.0. Berkas warisan mempertahankan hak masing-masing: Habring "
        ...
        "<p>Integrasi stokastik proksimal/cermin/minibatch dan reduksi varians yang lebih luas, ketaksamaan "
        "variasional dan operator monoton maksimal/resolven, penilaian kursus, laboratorium, kapstone, dan "
        "penutupan aksesibilitas bertag masih terbuka. PDF dapat dicari tetapi belum bertag; HTML semantik memakai "
        "runtime MathJax 3 jarak jauh yang dipatok. Bantuan produksi dan QA: "
        f"{MODEL_ID}, atas arahan pengguna repositori.</p>"
    )
    metadata = {
        "title": "Optimisasi Lanjut dan Analisis Konveks — Edisi Bahasa Indonesia (id-ID): Reduksi Varians untuk SAA; Kursus Parsial",
        ...
        "rights": [
            {
                ...
                "description": {"en": "No blanket license is asserted for this mixed-source record; consult RIGHTS_AND_PROVENANCE_BECKER_03.md and the component ledger."},
        ...
        "related_identifiers": [
            ...
            {"identifier": "10.48550/arXiv.1407.0202", "relation_type": {"id": "references"}, "resource_type": {"id": "publication-article"}, "scheme": "doi"},
        ],
        "subjects": [
            {"subject": value}
            for value in (
                "Bahasa Indonesia", "id-ID", "convex analysis", "convex optimization",
                "sample average approximation", "SAGA", "variance reduction",
...
def validate_metadata(metadata: dict) -> None:
    ...
    for required in (
        ...
        "3.585",
        "dua snapshot backend becker-02 dan lima alat bantu rilis lama",
        "batas 100 berkas zenodo",
        "aaron defazio",
        "pdf-nya tidak didistribusikan",
    ):
...
def build_local() -> dict:
    ...
    if len(parent) != 99 or len(inherited) != 92:
    ...
    manifest = {
        "schema": "o015-zenodo-becker-03-release-manifest-v1",
        "publication_date": "2026-08-25",
        "status": "partial",
        ...
        "boundary": {
            "ranges": ["2971-2988"],
            "english_witness_sha256": "66f243b97cd379b73d217c6a3e424db688f8ace246852cb24f78108c53186607",
            "excluded_adjacent_material": [
                "earlier stochastic-gradient material before line 2971",
                "document terminator and trailing blank lines after line 2988",
            ],
            "corrections": "O015-BECKER-ADV-0020 through O015-BECKER-ADV-0024",
            "primary_result_witness": "Defazio, Bach, and Lacoste-Julien, arXiv:1407.0202v3; local evidence only; not redistributed",
        },
        "backend": {
            "protected_records": 3430,
            "added_records": 155,
            "records": 3585,
            "jsonl_sha256": EXPECTED_LOCAL["backend-records-2026.08.25-becker-03.jsonl"][1],
            "csv_sha256": EXPECTED_LOCAL["backend-records-2026.08.25-becker-03.csv"][1],
        },
        ...
        "expected_public_file_count": 100,
    ...
    if len(paths) != 8 or len(expected) != 100 or set(expected) != set(inherited) | set(paths):
        raise RuntimeError("final 100-file namespace arithmetic mismatch")
    return {
        "result": "pass",
        "parent_files": len(parent),
        "removed_files": len(REPLACED_PARENT_FILES),
...
def session(*, authenticated: bool = True) -> requests.Session:
    ...
    client.headers.update({"User-Agent": "o015-becker-03-publisher/1"})
    if authenticated:
...
def state() -> dict:
    ...
    if (
        value.get("schema") != "o015-zenodo-becker-03-draft-v1"
...
def save_state(record: dict) -> dict:
    ...
    receipt = {
        "schema": "o015-zenodo-becker-03-draft-v1",
        "status": status,
        "draft_id": identifier,
        "draft_doi": doi,
        "version": VERSION,
...
def validate_live_draft_identity(value: dict, record: str) -> dict:
    ...
    if marker not in allowed:
        raise RuntimeError("live draft title/version is not the parent or Becker-03 release")
    return value
...
def find_unbound_lineage_draft(client: requests.Session) -> dict:
    ...
    if len(candidates) != 1:
        raise RuntimeError(
            f"expected one recoverable Becker-03 lineage draft, found {len(candidates)}"
        )
...
def ensure_imported_and_pruned(client: requests.Session, record: str) -> dict[str, dict]:
    ...
    missing_inherited = inherited_names - actual
    if unexpected or missing_inherited or actual & REPLACED_PARENT_FILES:
        raise RuntimeError(
            "bounded deletion/resume did not converge: "
            f"unexpected={sorted(unexpected)}, missing_inherited={sorted(missing_inherited)}, "
            f"superseded_remaining={sorted(actual & REPLACED_PARENT_FILES)}"
        )
    return entries
...
def upload() -> dict:
    ...
    order = {
        "D90-BECKER-03-reduksi-varians-id.pdf": 0,
        "D90-BECKER-03-reduksi-varians-id.html": 1,
        "ADVANCED_OPTIMIZATION_CONVEX_ANALYSIS_ID_BECKER_03_2026.08.25.zip": 2,
        "backend-records-2026.08.25-becker-03.jsonl": 3,
        "backend-records-2026.08.25-becker-03.csv": 4,
        RIGHTS_PATH.name: 5,
    ...
    validate_draft()
    return {"result": "pass", "draft_id": record, "addition_count": 8, "file_count": 100}
...
def validate_draft() -> dict:
    ...
    if metadata_payload().get("files", {}).get("default_preview") != PRIMARY_PDF:
        raise RuntimeError("local publication payload lacks the Becker-03 PDF preview")
    entries = remote_entries(client, record)
    ...
    if file_state.get("default_preview") != PRIMARY_PDF:
        raise RuntimeError("draft file state lacks the exact Becker-03 preview request")
    ...
    if draft_preview != PRIMARY_PDF:
        ...
        # only a known PDF in the exact 100-file namespace; publish() then
...
def public_default_preview(client: requests.Session, record: str) -> str:
    ...
    if preview != PRIMARY_PDF:
        raise RuntimeError("public file-state does not select the Becker-03 PDF")
    return preview
...
def repair_default_preview(record: dict) -> dict:
    ...
    if (
        ...
        raise RuntimeError("refusing preview repair outside the published Becker-03 lineage")
    expected = expected_inventory()
    public = public_entries(record)
    if set(public) != set(expected) or len(public) != 100:
    ...
    if file_state.get("default_preview") != PRIMARY_PDF:
        raise RuntimeError("same-record repair did not bind the Becker-03 preview request")
    try:
    ...
    write_json(
        PREVIEW_REPAIR_PATH,
        {
            "schema": "o015-zenodo-becker-03-default-preview-repair-v1",
            ...
            "file_count_before": 100,
            "file_count_after": 100,
...
def readback(wait: bool = False, repair_preview: bool = False) -> dict:
    ...
    for item in verified:
        item["disposition"] = "inherited_unchanged" if item["filename"] in inherited_names else "becker_03_addition"
    serialized = json.dumps(record["metadata"], ensure_ascii=False)
    receipt = {
        "schema": "o015-zenodo-becker-03-public-readback-v1",
        ...
        "addition_file_count": sum(item["disposition"] == "becker_03_addition" for item in verified),
        "removed_parent_files": sorted(REPLACED_PARENT_FILES),
        "removed_parent_file_count": len(REPLACED_PARENT_FILES),
        "files": verified,
    ...
    if (
        ...
        or receipt["file_count"] != 100
        or receipt["inherited_file_count"] != 92
...
def verify_draft_closure() -> dict:
    ...
    if current.get("status") != "published" or not current.get("draft_doi"):
        raise RuntimeError("published Becker-03 state is not available")
    record = str(current["draft_id"])
    client = session()
    ...
    receipt = {
        "schema": "o015-zenodo-becker-03-draft-closure-v1",
        "verified_at_utc": datetime.now(timezone.utc).isoformat(),
        "result": "pass",
        "record_id": record,
