#!/usr/bin/env python3
"""Fail-closed Zenodo publisher and anonymous readback for MIT L07.
...
READBACK_PATH = HERE / "zenodo-public-readback-mit-l07.json"
EXPECTED_TITLE = "Optimisasi Lanjut dan Analisis Konveks - Edisi Bahasa Indonesia (id-ID): Checkpoint MIT 6.253 Kuliah 3, Halaman 29-38 (Belum Lengkap)"
...
def session(authenticated: bool = True) -> requests.Session:
    ...
    client.headers["User-Agent"] = "O015-id-ID-MIT-L07-preservation/1.0"
    if authenticated:
...
def save_state(record: dict) -> dict:
    receipt = {
        "schema": "o015-zenodo-mit-l07-draft-receipt-v1",
...
def state() -> dict:
    if not STATE_PATH.is_file():
        raise RuntimeError("No local MIT-L07 draft receipt; run prepare first")
...
def validate_metadata_shape(metadata: dict) -> None:
    ...
    if title != EXPECTED_TITLE or metadata.get("version") != build.VERSION or metadata.get("publication_date") != "2026-08-24":
        raise RuntimeError("MIT-L07 title/version/date metadata mismatch")
    ...
    for required in ("halaman 29-38", "halaman 39", "belum lengkap", "58 berkas induk", "cc by-nc-sa 4.0", "athena"):
...
def validate_record_shape(record: dict, require_preview: bool = True) -> None:
    ...
    if require_preview and record.get("files", {}).get("default_preview") != build.READER_PATHS[0].name:
        raise RuntimeError("remote record does not make the L07 PDF its default preview")
...
def template_payload(include_preview: bool) -> dict:
    ...
    if payload.get("files", {}).get("default_preview") != build.READER_PATHS[0].name:
        raise RuntimeError("local metadata template lacks the required L07 PDF preview")
...
def ensure_inherited_imported(client: requests.Session, record_id: str) -> dict[str, dict]:
    ...
    for _ in range(90):
        ...
        if inherited.issubset(set(entries)):
            ...
            return entries
        time.sleep(1)
    raise RuntimeError("files-import did not expose all 58 inherited files")
...
def verify_parent_public() -> dict:
    ...
    if str(latest.get("id")) != build.PARENT_RECORD_ID or latest.get("status") != "published":
        raise RuntimeError("concept latest-version resolution does not equal required parent 22073743")
    ...
    if set(entries) != set(expected):
        raise RuntimeError("parent inventory differs from the frozen 58-file readback")
...
def readback(wait_for_publication: bool = False) -> dict:
    ...
    receipt = {
        ...
        "addition_file_count": sum(item["disposition"] == "mit_l07_addition" for item in verified),
