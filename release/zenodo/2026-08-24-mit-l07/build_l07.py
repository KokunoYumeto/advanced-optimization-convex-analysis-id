#!/usr/bin/env python3
"""Build and verify the deterministic, additive MIT-L07 preservation delta.

This builder is deliberately local and fail-closed. It binds the complete
Lecture 3 boundary (MIT 6.253 complete-notes PDF pages 29--38), the validated
1,820-record backend, and the published 58-file L06 parent. It performs no
network, credential, Git, publication, or mutable-control operation.
"""
...
PARENT_RECORD_DOI = "10.5281/zenodo.22073743"
CONCEPT_ID = "22059741"
...
NEXT_SOURCE_PAGE = 39
BACKEND_RECORD_COUNT = 1820
PROTECTED_BACKEND_RECORD_COUNT = 1714
NEW_BACKEND_RECORD_COUNT = 106

BUNDLE_NAME = "ADVANCED_OPTIMIZATION_CONVEX_ANALYSIS_ID_SOURCE_BACKEND_2026.08.24_MIT_L07_DELTA.zip"
MANIFEST_NAME = "release-manifest-mit-l07.json"
SUMS_NAME = "SHA256SUMS-mit-l07"
EXPECTED_INHERITED_COUNT = 58
EXPECTED_ADDITION_COUNT = 8
EXPECTED_RELEASE_COUNT = EXPECTED_INHERITED_COUNT + EXPECTED_ADDITION_COUNT

READER_PATHS = [
    ROOT / "output/pdf/D90-MIT-07-kuliah-3-fungsi-konveks-terdiferensial-caratheodory-id.pdf",
    ROOT / "output/html/D90-MIT-07-kuliah-3-fungsi-konveks-terdiferensial-caratheodory-id.html",
]
RELEASE_DOCS = ["README_MIT_L07.md", "README_RELEASE_MIT_L07.md", "LICENSE_MIT_L07.md"]

# Compact continuation closure only. Mutable global controls and authority bytes
# are intentionally absent; two immutable, boundary-specific evidence files are
# included because the L07 receipts bind them directly.
BUNDLE_ROOT_PATHS = [
    "backend/backend_schema.json",
    "backend/records.jsonl",
    "backend/records.csv",
    "source/en/mit-07-lecture-3-differentiable-convex-functions-caratheodory-semantic-witness.md",
    "source/id-ID/mit-07-kuliah-3-fungsi-konveks-terdiferensial-caratheodory-id.md",
    "source/id-ID/mit-l07.css",
    "source/id-ID/mit-l07-preamble.tex",
    "source/id-ID/mit-l07-pdf-filter.lua",
    "source/id-ID/mit-l07-before-body.html",
    "source/id-ID/mit-l07-after-body.html",
    "qa/build_mit_l07.py",
    "qa/validate_mit_l07.py",
    "qa/MIT_L07_VALIDATION.json",
    "qa/MIT_L07_BROWSER_QA.json",
    "qa/MIT_L07_VISUAL_QA.json",
    "qa/MIT_L07_INDEPENDENT_REREVIEW.md",
    "qa/extend_backend_mit_l07.py",
    "qa/validate_backend_mit_l07.py",
    "qa/MIT_L07_BACKEND_VALIDATION.json",
    "00_control/MIT_L07_LECTURE_3_PAGES_029-038_BOUNDARY_CENSUS.md",
    "00_control/MIT_L07_CORRECTION_SNAPSHOT.jsonl",
...
def rooted_identity(relative: str) -> dict[str, object]:
...
def inherited_inventory() -> dict[str, dict[str, object]]:
    ...
    if (
        ...
        raise RuntimeError("frozen readback does not identify the published L06 parent in the required concept")
    ...
    if (
        ...
        raise RuntimeError("parent readback does not prove exactly 58 unique public files")
    return result
...
def validate_template() -> dict:
    ...
    if template.get("files", {}).get("default_preview") != READER_PATHS[0].name:
        raise RuntimeError("metadata template must make the L07 PDF the default preview")
    ...
    for required in ("halaman 29-38", "halaman 39", "belum lengkap", "58 berkas induk", "cc by-nc-sa 4.0"):
...
def validate_release_docs() -> None:
    ...
    for required in ("halaman 29", "halaman 39", "belum lengkap", "cc by-nc-sa 4.0", "athena", "1.820"):
        if required not in lowered:
...
def validate_input_lock() -> dict:
    ...
    if (
        lock.get("schema") != "o015-mit-l07-release-input-lock-v1"
        or lock.get("boundary", {}).get("source_pages") != SOURCE_PAGES
        or lock.get("boundary", {}).get("next_source_page") != NEXT_SOURCE_PAGE
        or lock.get("backend_record_count") != BACKEND_RECORD_COUNT
        or lock.get("protected_backend_record_count") != PROTECTED_BACKEND_RECORD_COUNT
        or lock.get("new_backend_record_count") != NEW_BACKEND_RECORD_COUNT
    ):
        raise RuntimeError("L07 release input lock has a different boundary or backend count")
    files = lock.get("files", {})
    if not isinstance(files, dict) or len(files) != 22:
        raise RuntimeError("L07 input lock must bind exactly 22 final files")
...
def validate_lane_closure() -> dict[str, object]:
    ...
    files = lock["files"]
    content = read_json(ROOT / "qa/MIT_L07_VALIDATION.json")
    browser = read_json(ROOT / "qa/MIT_L07_BROWSER_QA.json")
    visual = read_json(ROOT / "qa/MIT_L07_VISUAL_QA.json")
    backend = read_json(ROOT / "qa/MIT_L07_BACKEND_VALIDATION.json")
    rereview_path = ROOT / "qa/MIT_L07_INDEPENDENT_REREVIEW.md"
    ...
    if (
        ...
        or content.get("boundary", {}).get("source_items") != 16
        or content.get("boundary", {}).get("nested_items") != 14
        or content.get("boundary", {}).get("source_displays") != 13
        or content.get("boundary", {}).get("source_figures") != 4
        or content.get("boundary", {}).get("source_figure_panels") != 6
        ...
        or content.get("rights", {}).get("athena_source_figure_blocks_omitted") != 4
        or content.get("rights", {}).get("athena_source_figure_panels_omitted") != 6
    ):
        raise RuntimeError("final L07 content validation closure failed")
    if (
        content.get("build", {}).get("html_sha256") != files[READER_PATHS[1].relative_to(ROOT).as_posix()]["sha256"]
        or content.get("build", {}).get("pdf_sha256") != files[READER_PATHS[0].relative_to(ROOT).as_posix()]["sha256"]
        or content.get("build", {}).get("deterministic_rebuilds") != 2
        or content.get("formula_sequence_match") is not True
    ):
        raise RuntimeError("L07 deterministic reader identities/formula sequence are not final")
    ...
    html_lock = files[READER_PATHS[1].relative_to(ROOT).as_posix()]
    pdf_lock = files[READER_PATHS[0].relative_to(ROOT).as_posix()]
    if (
        browser.get("result") != "pass"
        or browser.get("surface", {}).get("path") != READER_PATHS[1].relative_to(ROOT).as_posix()
        or browser.get("surface", {}).get("bytes") != html_lock["bytes"]
        or browser.get("surface", {}).get("sha256") != html_lock["sha256"]
        or browser.get("desktop", {}).get("horizontal_overflow") is not False
        or browser.get("mobile", {}).get("horizontal_overflow") is not False
        or browser.get("console_warnings_or_errors") != []
    ):
        raise RuntimeError("final L07 browser validation closure failed")
    if (
        visual.get("result") != "pass"
        or visual.get("surface", {}).get("path") != READER_PATHS[0].relative_to(ROOT).as_posix()
        or visual.get("surface", {}).get("bytes") != pdf_lock["bytes"]
        or visual.get("surface", {}).get("sha256") != pdf_lock["sha256"]
        or visual.get("render", {}).get("pages") != 4
    ):
        raise RuntimeError("final L07 visual validation closure failed")
    if (
        "P1=0, P2=0, P3=0" not in rereview
        or lock["files"]["source/en/mit-07-lecture-3-differentiable-convex-functions-caratheodory-semantic-witness.md"]["sha256"] not in rereview
        or lock["files"]["source/id-ID/mit-07-kuliah-3-fungsi-konveks-terdiferensial-caratheodory-id.md"]["sha256"] not in rereview
    ):
        raise RuntimeError("independent rereview does not bind a clean L07 disposition")

    expected_segments = [f"d90.mit.ocw-6.253.l07.p{page:03d}" for page in SOURCE_PAGES]
    if (
        ...
        or backend.get("protected_baseline", {}).get("preserved_record_count") != PROTECTED_BACKEND_RECORD_COUNT
        or backend.get("protected_baseline", {}).get("raw_bytes_reconstructed_exactly") is not True
        ...
        raise RuntimeError("final L07 backend validation closure failed")
    ...
    for item in backend.get("canonical_artifacts", {}).values():
        relative = item.get("path")
        if not relative:
            raise RuntimeError("backend receipt contains an unbound canonical identity")
        require_identity(ROOT / relative, item, f"backend canonical identity {relative}")

    return {
        "result": "pass",
        "source_pages": SOURCE_PAGES,
        "next_source_page": NEXT_SOURCE_PAGE,
        "content_validation": identity(ROOT / "qa/MIT_L07_VALIDATION.json"),
        "browser_validation": identity(ROOT / "qa/MIT_L07_BROWSER_QA.json"),
        "visual_validation": identity(ROOT / "qa/MIT_L07_VISUAL_QA.json"),
        "independent_rereview": identity(rereview_path),
        "backend_validation": identity(ROOT / "qa/MIT_L07_BACKEND_VALIDATION.json"),
        "backend": {
            "record_count": BACKEND_RECORD_COUNT,
            "protected_record_count": PROTECTED_BACKEND_RECORD_COUNT,
            "new_record_count": NEW_BACKEND_RECORD_COUNT,
...
def bundle_inputs() -> list[tuple[str, Path]]:
    ...
    pairs.append(("release-notes/release-input-lock-mit-l07.json", INPUT_LOCK_PATH))
...
def verify_bundle(payload: bytes) -> dict[str, object]:
    with zipfile.ZipFile(io.BytesIO(payload), "r") as archive:
        ...
        if (
            ...
            or manifest.get("backend_record_count") != BACKEND_RECORD_COUNT
...
def build_bundle(lane_closure: dict[str, object]) -> tuple[Path, dict[str, object]]:
    ...
    inner = {
        "schema": "o015-mit-l07-delta-bundle-v1",
        "version": VERSION,
        "scope": "MIT 6.253 complete-notes Lecture 3, pages 29-38; compact semantic source, build, QA, backend, boundary evidence, and license closure",
        "status": STATUS,
    ...
    with tempfile.NamedTemporaryFile(prefix=".mit-l07-delta-", suffix=".zip", dir=HERE, delete=False) as handle:
...
def build_release_metadata(bundle_verification: dict[str, object], lane_closure: dict[str, object]) -> None:
    ...
    manifest = {
        ...
        "admitted_boundary": "MIT 6.253 complete-notes Lecture 3, PDF pages 29-38, L07",
        "next_boundary": "MIT 6.253 complete-notes PDF page 39, Lecture 4",
        "backend_record_count": BACKEND_RECORD_COUNT,
        "protected_backend_record_count": PROTECTED_BACKEND_RECORD_COUNT,
        "new_backend_record_count": NEW_BACKEND_RECORD_COUNT,
        "component_rights": COMPONENT_RIGHTS,
...
def validate_local_release(require_draft_binding: bool = False) -> dict[str, object]:
    ...
    if (
        ...
        or manifest.get("next_boundary") != "MIT 6.253 complete-notes PDF page 39, Lecture 4"
        or manifest.get("component_rights") != COMPONENT_RIGHTS
        or manifest.get("complete_corpus") is not False
        or manifest.get("status") != STATUS
        or manifest.get("model_provenance") != MODEL_ID
