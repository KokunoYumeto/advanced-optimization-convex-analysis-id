#!/usr/bin/env python3
"""Prepare, upload, publish, and anonymously read back the MIT-L03 version."""
...
import build_l03 as build


HERE = Path(__file__).resolve().parent
API = "https://zenodo.org/api"
CREDENTIAL = Path.home() / "Documents" / "Obsidian notes" / "New zenodo token.md"
TEMPLATE_PATH = HERE / "zenodo-record-mit-l03.json"
STATE_PATH = HERE / "zenodo-draft-mit-l03.json"
READBACK_PATH = HERE / "zenodo-public-readback-mit-l03.json"
EXPECTED_TITLE = "Optimisasi Lanjut dan Analisis Konveks - Edisi Bahasa Indonesia (id-ID): Checkpoint MIT 6.253 L03 (Belum Lengkap)"
...
def session(authenticated: bool = True) -> requests.Session:
    ...
    client.headers["User-Agent"] = "O015-id-ID-MIT-L03-preservation/1.0"
    if authenticated:
...
def save_state(record: dict) -> dict:
    receipt = {"schema": "o015-zenodo-mit-l03-draft-receipt-v1", "parent_record_id": build.PARENT_RECORD_ID, "parent_record_doi": build.PARENT_RECORD_DOI, "concept_id": build.CONCEPT_ID, "concept_doi": build.CONCEPT_DOI, "draft_id": str(record["id"]), "draft_doi": draft_doi(record), "status": record.get("status"), "title": record.get("metadata", {}).get("title"), "version": record.get("metadata", {}).get("version")}
...
def state() -> dict:
    if not STATE_PATH.is_file():
        raise RuntimeError("No local MIT-L03 draft receipt; run prepare first")
...
def validate_metadata_shape(metadata: dict) -> None:
    ...
    if title != EXPECTED_TITLE or metadata.get("version") != build.VERSION or metadata.get("publication_date") != "2026-08-23":
        raise RuntimeError("MIT-L03 title/version/date metadata mismatch")
    ...
    for required in ("halaman 14", "halaman 15", "belum lengkap", "induk", "cc by-nc-sa 4.0"):
...
def verify_parent_public() -> dict:
    ...
    prior = json.loads(build.PREVIOUS_READBACK.read_text(encoding="utf-8"))
    if prior.get("inherited_identity") != "pass" or any(item.get("public_byte_identity") != "pass" for item in prior.get("files", [])):
        raise RuntimeError("frozen parent receipt lacks complete prior public-byte verification")
...
def upload() -> dict:
    ...
    for path in sorted(build.addition_paths(), key=lambda item: item.name):
        ...
        existing = remote_entries(client)
    validate_draft()
    return {"result": "pass", "draft_id": draft_id(), "addition_count": 8}
