#!/usr/bin/env python3
"""Fail-closed Zenodo publisher and anonymous readback for MIT L08.

Importing this module performs no network or credential operation. Every
mutating action first runs the deterministic local release gate. A successful
publish is immediately followed by anonymous inventory and full-byte SHA-256
readback; draft creation or upload alone is never reported as publication.
"""
...
READBACK_PATH = HERE / "zenodo-public-readback-mit-l08.json"
EXPECTED_TITLE = "Optimisasi Lanjut dan Analisis Konveks - Edisi Bahasa Indonesia (id-ID): Checkpoint MIT 6.253 Kuliah 4, Halaman 39-49 (Belum Lengkap)"
...
def session(authenticated: bool = True) -> requests.Session:
    ...
    client.headers["User-Agent"] = "O015-id-ID-MIT-L08-preservation/1.0"
    if authenticated:
...
def save_state(record: dict) -> dict:
    receipt = {
        "schema": "o015-zenodo-mit-l08-draft-receipt-v1",
...
def state() -> dict:
    if not STATE_PATH.is_file():
        raise RuntimeError("No local MIT-L08 draft receipt; run prepare first")
...
def validate_metadata_shape(metadata: dict) -> None:
    ...
    if title != EXPECTED_TITLE or metadata.get("version") != build.VERSION or metadata.get("publication_date") != "2026-08-24":
        raise RuntimeError("MIT-L08 title/version/date metadata mismatch")
    if (
        "TTP" in title
        ...
        raise RuntimeError("organizational metadata must contain one short TTP entry and no expansion")
    if serialized.count(build.MODEL_ID) != 1:
        raise RuntimeError("exact model provenance must occur once")
    lowered = description.lower()
    for required in ("halaman 39-49", "halaman 50", "belum lengkap", "66 berkas induk", "cc by-nc-sa 4.0", "lima blok gambar", "o015-mit-sem-0011"):
...
def validate_record_shape(record: dict, require_preview: bool = True) -> None:
    ...
    if require_preview and record.get("files", {}).get("default_preview") != build.READER_PATHS[0].name:
        raise RuntimeError("remote record does not make the L08 PDF its default preview")
...
def template_payload(include_preview: bool) -> dict:
    ...
    if payload.get("files", {}).get("default_preview") != build.READER_PATHS[0].name:
        raise RuntimeError("local metadata template lacks the required L08 PDF preview")
...
def verify_parent_public() -> dict:
    ...
    if str(latest.get("id")) != build.PARENT_RECORD_ID or latest.get("status") != "published":
        raise RuntimeError("concept latest-version resolution does not equal required parent 22074102")
    ...
    if set(entries) != set(expected):
        raise RuntimeError("parent inventory differs from the frozen 66-file readback")
...
def ensure_inherited_imported(client: requests.Session, record_id: str) -> dict[str, dict]:
    ...
    for _ in range(90):
        ...
        if inherited.issubset(set(entries)):
            ...
            return entries
        time.sleep(1)
    raise RuntimeError("files-import did not expose all 66 inherited files")
...
def prepare() -> dict:
    ...
    if response.status_code == 409:
        raise RuntimeError("Zenodo reports an existing new-version draft; refusing a duplicate without its local receipt")
    response.raise_for_status()
...
def readback(wait_for_publication: bool = False) -> dict:
    ...
    if set(entries) != set(expected) or len(entries) != build.EXPECTED_RELEASE_COUNT:
        raise RuntimeError("anonymous public 74-file inventory mismatch")
    verified = verify_expected_bytes(client, entries, expected)
    inherited_names = set(build.inherited_inventory())
    for item in verified:
        item["disposition"] = "inherited_unchanged" if item["filename"] in inherited_names else "mit_l08_addition"
    serialized = json.dumps(record["metadata"], ensure_ascii=False)
    receipt = {
        "schema": "o015-zenodo-mit-l08-public-readback-v1",
        "record_id": draft_id(),
        ...
        "addition_file_count": sum(item["disposition"] == "mit_l08_addition" for item in verified),
...
def main() -> None:
    ...
    result = {"prepare": prepare, "upload": upload, "validate": validate_draft, "publish": publish, "readback": readback, "release": release}[args.action]()
