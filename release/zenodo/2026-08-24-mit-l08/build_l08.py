#!/usr/bin/env python3
"""Build and verify the deterministic, additive MIT-L08 preservation delta.

This local fail-closed builder binds MIT 6.253 Lecture 4 (complete-notes PDF
pages 39--49), the validated 1,957-record backend, and the published 66-file
L07 parent. It performs no network, credential, Git, publication, or
mutable-control operation.
"""
...
ROOT = HERE.parents[2]
PREVIOUS_READBACK = HERE.parent / "2026-08-24-mit-l07" / "zenodo-public-readback-mit-l07.json"
TEMPLATE_PATH = HERE / "zenodo-record-mit-l08.json"
INPUT_LOCK_PATH = HERE / "release-input-lock-mit-l08.json"
STATE_PATH = HERE / "zenodo-draft-mit-l08.json"
FIXED_ZIP_TIME = (2026, 8, 24, 0, 0, 0)

PARENT_RECORD_ID = "22074102"
PARENT_RECORD_DOI = "10.5281/zenodo.22074102"
CONCEPT_ID = "22059741"
...
BUNDLE_NAME = "ADVANCED_OPTIMIZATION_CONVEX_ANALYSIS_ID_SOURCE_BACKEND_2026.08.24_MIT_L08_DELTA.zip"
MANIFEST_NAME = "release-manifest-mit-l08.json"
SUMS_NAME = "SHA256SUMS-mit-l08"
EXPECTED_INHERITED_COUNT = 66
EXPECTED_ADDITION_COUNT = 8
EXPECTED_RELEASE_COUNT = EXPECTED_INHERITED_COUNT + EXPECTED_ADDITION_COUNT

READER_PATHS = [
    ROOT / "output/pdf/D90-MIT-08-kuliah-4-interior-relatif-penutupan-kontinuitas-id.pdf",
    ROOT / "output/html/D90-MIT-08-kuliah-4-interior-relatif-penutupan-kontinuitas-id.html",
]
RELEASE_DOCS = ["README_MIT_L08.md", "README_RELEASE_MIT_L08.md", "LICENSE_MIT_L08.md"]
...
def inherited_inventory() -> dict[str, dict[str, object]]:
    ...
    if (
        ...
        raise RuntimeError("frozen readback does not identify the published L07 parent")
    ...
    if (
        ...
        raise RuntimeError("parent readback does not prove exactly 66 unique public files")
    return result
...
def validate_template() -> dict:
    ...
    if template.get("files", {}).get("default_preview") != READER_PATHS[0].name:
        raise RuntimeError("metadata template must make the L08 PDF the default preview")
    ...
    if (
        ...
        raise RuntimeError("metadata may contain only one short TTP organizational contributor and no expansion")
    ...
    for required in ("halaman 39-49", "halaman 50", "belum lengkap", "66 berkas induk", "cc by-nc-sa 4.0", "lima blok gambar"):
...
def validate_release_docs() -> None:
    ...
    if "TTP" in combined or FORBIDDEN_ORG_EXPANSION.casefold() in combined.casefold():
        raise RuntimeError("release documents must not add a TTP label or expansion")
    lowered = combined.lower()
    for required in ("halaman 39", "halaman 50", "belum lengkap", "cc by-nc-sa 4.0", "athena", "1.957"):
        if required not in lowered:
...
def validate_input_lock() -> dict:
    ...
    if (
        lock.get("schema") != "o015-mit-l08-release-input-lock-v1"
        ...
        raise RuntimeError("L08 release input lock has a different boundary or backend count")
    files = lock.get("files", {})
    if not isinstance(files, dict) or len(files) != 22:
        raise RuntimeError("L08 input lock must bind exactly 22 final files")
...
def validate_lane_closure() -> dict[str, object]:
    ...
    browser = read_json(ROOT / "qa/MIT_L08_BROWSER_QA.json")
    visual = read_json(ROOT / "qa/MIT_L08_VISUAL_QA.json")
    backend = read_json(ROOT / "qa/MIT_L08_BACKEND_VALIDATION.json")
    rereview_path = ROOT / "qa/MIT_L08_INDEPENDENT_REREVIEW.md"
    ...
    if (
        ...
        or boundary.get("source_items") != 27
        or boundary.get("nested_items") != 16
        or boundary.get("source_displays") != 26
        or boundary.get("source_figures") != 5
        or boundary.get("source_figure_panels") != 5
        ...
        or content.get("rights", {}).get("athena_source_figure_blocks_omitted") != 5
        or content.get("rights", {}).get("athena_source_figure_panels_omitted") != 5
    ):
        raise RuntimeError("final L08 content validation closure failed")
    ...
    if (
        ...
        or content.get("formula_inventory", {}).get("witness_display_blocks") != 26
        or content.get("formula_inventory", {}).get("target_display_blocks") != 26
    ):
        raise RuntimeError("L08 deterministic reader/formula closure failed")
    ...
    if (
        ...
        or browser.get("build", {}).get("html_path") != html_relative
        or browser.get("build", {}).get("html_bytes") != html_lock["bytes"]
        or browser.get("build", {}).get("html_sha256") != html_lock["sha256"]
        or browser.get("desktop", {}).get("horizontal_overflow") is not False
        or browser.get("mobile", {}).get("horizontal_overflow") is not False
        or browser.get("desktop", {}).get("console_warnings_or_errors") != []
        or browser.get("mobile", {}).get("console_warnings_or_errors") != []
    ):
        raise RuntimeError("final L08 browser validation closure failed")
    if (
        visual.get("inspection", {}).get("result") != "pass"
        ...
        or visual.get("pdf", {}).get("pages") != 6
    ):
        raise RuntimeError("final L08 visual validation closure failed")
    if (
        not re.search(r"P1\s*=\s*0\s*,\s*P2\s*=\s*0\s*,\s*P3\s*=\s*0", rereview)
        or files["source/en/mit-08-lecture-4-relative-interior-closure-continuity-semantic-witness.md"]["sha256"] not in rereview
        or files["source/id-ID/mit-08-kuliah-4-interior-relatif-penutupan-kontinuitas-id.md"]["sha256"] not in rereview
    ):
        raise RuntimeError("independent rereview does not bind a clean L08 disposition")
    expected_segments = [f"d90.mit.ocw-6.253.l08.p{page:03d}" for page in SOURCE_PAGES]
    if (
        ...
        raise RuntimeError("final L08 backend validation closure failed")
    ...
    correction_lock = files["00_control/MIT_L08_CORRECTION_SNAPSHOT.jsonl"]
    ...
    return {
        ...
        "content_validation": identity(ROOT / "qa/MIT_L08_VALIDATION.json"),
        "browser_validation": identity(ROOT / "qa/MIT_L08_BROWSER_QA.json"),
        "visual_validation": identity(ROOT / "qa/MIT_L08_VISUAL_QA.json"),
        "independent_rereview": identity(rereview_path),
        "backend_validation": identity(ROOT / "qa/MIT_L08_BACKEND_VALIDATION.json"),
        "backend": {
...
def bundle_inputs() -> list[tuple[str, Path]]:
    ...
    pairs.append(("release-notes/release-input-lock-mit-l08.json", INPUT_LOCK_PATH))
...
def verify_bundle(payload: bytes) -> dict[str, object]:
    ...
    with zipfile.ZipFile(io.BytesIO(payload), "r") as archive:
        ...
        if set(names) != {"DELTA_BUNDLE_MANIFEST.json", *(entry["path"] for entry in entries)} or manifest.get("entry_count") != len(entries):
            raise RuntimeError("delta ZIP inventory differs from internal manifest")
        if {entry["path"] for entry in entries} != set(live_inputs):
            raise RuntimeError("delta ZIP payload inventory differs from the current live input closure")
        ...
        permitted_controls = {"00_control/MIT_L08_LECTURE_4_BOUNDARY_CENSUS.md", "00_control/MIT_L08_CORRECTION_SNAPSHOT.jsonl"}
        ...
        return {"entries": len(names), "manifest_entries_verified": len(entries), "integrity": "pass", "forbidden_entries": 0, "mutable_global_control_files": 0}
...
def build_bundle(lane_closure: dict[str, object]) -> tuple[Path, dict[str, object]]:
    ...
    inner = {
        "schema": "o015-mit-l08-delta-bundle-v1",
        "version": VERSION,
        "scope": "MIT 6.253 complete-notes Lecture 4, pages 39-49; compact semantic source, build, QA, backend, boundary evidence, and license closure",
        "status": STATUS,
    ...
    with tempfile.NamedTemporaryFile(prefix=".mit-l08-delta-", suffix=".zip", dir=HERE, delete=False) as handle:
...
def draft_identity() -> tuple[str | None, str | None]:
    ...
    if (
        data.get("schema") != "o015-zenodo-mit-l08-draft-receipt-v1"
        or data.get("parent_record_id") != PARENT_RECORD_ID
        or data.get("concept_id") != CONCEPT_ID
        or data.get("version") != VERSION
    ):
        raise RuntimeError("local draft receipt belongs to a different L08 lineage/version")
...
def build_release_metadata(bundle_verification: dict[str, object], lane_closure: dict[str, object]) -> None:
    ...
    manifest = {
        ...
        "admitted_boundary": "MIT 6.253 complete-notes Lecture 4, PDF pages 39-49, L08",
        "next_boundary": "MIT 6.253 complete-notes PDF page 50, Lecture 5",
        "backend_record_count": BACKEND_RECORD_COUNT,
        "protected_backend_record_count": PROTECTED_BACKEND_RECORD_COUNT,
        "new_backend_record_count": NEW_BACKEND_RECORD_COUNT,
        "component_rights": COMPONENT_RIGHTS,
...
def validate_local_release(require_draft_binding: bool = False) -> dict[str, object]:
    ...
    if (
        ...
        or manifest.get("next_boundary") != "MIT 6.253 complete-notes PDF page 50, Lecture 5"
        or manifest.get("component_rights") != COMPONENT_RIGHTS
        or manifest.get("complete_corpus") is not False
        or manifest.get("status") != STATUS
        or manifest.get("model_provenance") != MODEL_ID
    ...
    if require_draft_binding:
        ...
        if manifest.get("zenodo_record_id") != record_id or manifest.get("zenodo_record_doi") != record_doi:
            raise RuntimeError("release manifest is not bound to the prepared L08 draft")
