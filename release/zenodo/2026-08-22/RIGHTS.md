...
Christopher Griffin's *Nonlinear Programming* / Penn State MATH 555 is licensed under CC BY-NC-SA 3.0 US. The Indonesian Chapter 3 and 4 sources and standalone readers are separately identified translation/adaptation components under that license. They retain attribution, a change notice, noncommercial use, ShareAlike, the license link, and non-endorsement. Exact Chapter 3 changes are recorded as `O015-PENN-ADV-0004` through `O015-PENN-ADV-0024`; exact Chapter 4 changes are `O015-PENN-ADV-0025` through `O015-PENN-ADV-0037` in `00_control/ADVERSE_LEDGER.jsonl`.

The nine Maple listing inputs encountered across Chapters 3 and 4, and the separate legacy-code tree, are excluded because their reusable component rights are unclear. Chapter 3 contains six independently authored locale-neutral pseudocode bridges and bounded numerical witnesses. Chapter 4 replaces its three excluded inputs with independently authored backtracking and two-part gradient-ascent pseudocode. No Maple or legacy-code text is copied or attributed to Penn. The nine bundled vector figures and the bounded exact excerpts from the essential bundled `Math555.bbl` remain Penn-derived components under the same NC-SA handling, with their provenance and caveats recorded in `00_control/COMPONENT_RIGHTS.csv`.
