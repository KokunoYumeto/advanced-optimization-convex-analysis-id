#!/usr/bin/env python3
"""Build, publish, and anonymously verify the Becker-01 Zenodo checkpoint.
...
PARENT_RECORD_ID = "22088305"
PARENT_RECORD_DOI = "10.5281/zenodo.22088305"
CONCEPT_ID = "22059741"
CONCEPT_DOI = "10.5281/zenodo.22059741"
VERSION = "checkpoint-2026.08.25-becker-01-lagrange-slater-kkt"
MODEL_ID = "OpenAI Codex gpt-5.6-sol, Ultra"
FORBIDDEN_ORG_EXPANSION = "Translation and Transcription Project"
GITHUB_COMMIT = "7ff680f4079499c08f3b29780105f58279f519d5"
GITHUB_TREE = "a36c6a880f483681575c4b946d53a4ee0e493bf5"
PRIMARY_PDF = "D90-BECKER-01-dualitas-lagrange-slater-kkt-id.pdf"

PARENT_READBACK = (
    ROOT
    / "release"
    / "zenodo"
    / "2026-08-25-habring-v1-complete-spine"
    / "zenodo-public-readback-habring-spine.json"
)
STATE_PATH = HERE / "zenodo-draft-becker-01.json"
READBACK_PATH = HERE / "zenodo-public-readback-becker-01.json"
PREVIEW_REPAIR_PATH = HERE / "zenodo-default-preview-repair-becker-01.json"
MANIFEST_PATH = HERE / "release-manifest-becker-01-zenodo.json"
SUMS_PATH = HERE / "SHA256SUMS-becker-01"
RIGHTS_PATH = HERE / "RIGHTS_AND_PROVENANCE_BECKER_01.md"

CORE_PUBLIC_PATHS = {
    "D90-BECKER-01-dualitas-lagrange-slater-kkt-id.pdf": ROOT
    / "output/pdf/D90-BECKER-01-dualitas-lagrange-slater-kkt-id.pdf",
    "D90-BECKER-01-dualitas-lagrange-slater-kkt-id.html": ROOT
    / "output/html/D90-BECKER-01-dualitas-lagrange-slater-kkt-id.html",
    "ADVANCED_OPTIMIZATION_CONVEX_ANALYSIS_ID_BECKER_01_2026.08.25.zip": ROOT
    / "release/becker-01/2026-08-25/ADVANCED_OPTIMIZATION_CONVEX_ANALYSIS_ID_BECKER_01_2026.08.25.zip",
    "backend-records-2026.08.25-becker-01.jsonl": ROOT / "backend/records.jsonl",
    "backend-records-2026.08.25-becker-01.csv": ROOT / "backend/records.csv",
    "backend-schema.json": ROOT / "backend/backend_schema.json",
...
SUPERSEDED_DELTA_ZIPS = {
    "ADVANCED_OPTIMIZATION_CONVEX_ANALYSIS_ID_SOURCE_BACKEND_2026.08.23_MIT_L01_DELTA.zip",
    "ADVANCED_OPTIMIZATION_CONVEX_ANALYSIS_ID_SOURCE_BACKEND_2026.08.23_MIT_L02_DELTA.zip",
    "ADVANCED_OPTIMIZATION_CONVEX_ANALYSIS_ID_SOURCE_BACKEND_2026.08.23_MIT_L03_DELTA.zip",
    "ADVANCED_OPTIMIZATION_CONVEX_ANALYSIS_ID_SOURCE_BACKEND_2026.08.23_MIT_L04_L05_DELTA.zip",
    "ADVANCED_OPTIMIZATION_CONVEX_ANALYSIS_ID_SOURCE_BACKEND_2026.08.23_MIT_L06_DELTA.zip",
    "ADVANCED_OPTIMIZATION_CONVEX_ANALYSIS_ID_SOURCE_BACKEND_2026.08.24_MIT_L07_DELTA.zip",
    "ADVANCED_OPTIMIZATION_CONVEX_ANALYSIS_ID_SOURCE_BACKEND_2026.08.24_MIT_L08_DELTA.zip",
    "ADVANCED_OPTIMIZATION_CONVEX_ANALYSIS_ID_SOURCE_BACKEND_2026.08.24_MIT_L09_DELTA.zip",
    "ADVANCED_OPTIMIZATION_CONVEX_ANALYSIS_ID_SOURCE_BACKEND_2026.08.24_MIT_L10_DELTA.zip",
    "ADVANCED_OPTIMIZATION_CONVEX_ANALYSIS_ID_SOURCE_BACKEND_2026.08.24_MIT_L11_DELTA.zip",
}
...
def parent_inventory() -> dict[str, dict[str, object]]:
    ...
    if len(entries) != 100 or not SUPERSEDED_DELTA_ZIPS.issubset(entries):
...
def metadata_payload() -> dict:
    description = (
        ...
        "Kursus yang lebih besar tetap parsial. Versi ini menambahkan modul nonduplikatif Dualitas Lagrange, "
        "Slater, dan KKT dari repositori Stephen Becker pada commit 98ed6930084c435ba0f675f7646ced1f2fd8729e, "
        ...
        "<p>Modul mempertahankan lima rentang sumber tepat dan mengecualikan rentang program linear yang menjadi "
        "cakupan O018. Dua belas koreksi matematis diumumkan. Dua build PDF/HTML deterministik, sepuluh uji matematika "
        "terbuka, reflow desktop/tablet/seluler, inspeksi semua 12 halaman PDF, rereview independen, dan backend lulus. "
        "Backend mempertahankan 3.096 rekaman sebelumnya dan menambahkan 224 rekaman, menjadi 3.320. Sepuluh ZIP delta "
        "MIT lama diganti hanya pada versi baru ini oleh PDF/HTML Becker, paket kelanjutan ringkas, backend mutakhir, "
        "catatan hak, manifest, dan checksum; versi Zenodo terdahulu tetap tak berubah.</p>"
        "<p>Hak berlaku per komponen. Sumber Becker berlisensi MIT dan dikreditkan kepada Stephen Becker serta Mitchell "
        "Krock, dengan kredit sumber Boyd–Vandenberghe dan Bertsekas dipertahankan. Terjemahan, kata-kata koreksi, dan "
        ...
        "<p>Douglas–Rachford, variasi-reduksi yang rigor, metode stokastik proksimal/cermin/minibatch, ketaksamaan "
    ...
    metadata = {
        "title": "Optimisasi Lanjut dan Analisis Konveks — Edisi Bahasa Indonesia (id-ID): Dualitas Lagrange, Slater, dan KKT; Kursus Parsial",
        ...
        "rights": [
            {
                ...
                "description": {"en": "No blanket license is asserted for this mixed-source record; consult RIGHTS_AND_PROVENANCE_BECKER_01.md and the component ledger."},
        ...
        "subjects": [
            ...
            for value in (
                ...
                "Lagrangian duality", "Slater condition", "Karush-Kuhn-Tucker conditions",
...
def build_local() -> dict:
    ...
    if len(parent) != 100 or len(inherited) != 90:
    ...
    manifest = {
        "schema": "o015-zenodo-becker-01-release-manifest-v1",
        "publication_date": "2026-08-25",
        "status": "partial",
        ...
        "boundary": {
            "ranges": ["1263-1321", "1398-1405", "1414-1499", "1652-1726", "1731-1743"],
            "english_witness_sha256": "20335c054393ea43d8912046b6dbfa07f6018f9e16b889e4cd0f66abc064d565",
            "o018_exclusions": ["1322-1397", "1406-1413", "1727-1730"],
            "corrections": "O015-BECKER-ADV-0001 through O015-BECKER-ADV-0012",
        },
        "backend": {
            "protected_records": 3096,
            "added_records": 224,
            "records": 3320,
            "jsonl_sha256": EXPECTED_LOCAL["backend-records-2026.08.25-becker-01.jsonl"][1],
            "csv_sha256": EXPECTED_LOCAL["backend-records-2026.08.25-becker-01.csv"][1],
        },
    ...
    if len(paths) != 9 or len(expected) != 99 or set(expected) != set(inherited) | set(paths):
...
def session(*, authenticated: bool = True) -> requests.Session:
    ...
    client.headers.update({"User-Agent": "o015-becker-01-publisher/1"})
    if authenticated:
...
def state() -> dict:
    ...
    if (
        value.get("schema") != "o015-zenodo-becker-01-draft-v1"
...
def save_state(record: dict) -> dict:
    ...
    receipt = {
        "schema": "o015-zenodo-becker-01-draft-v1",
        "status": status,
        "draft_id": identifier,
        "draft_doi": doi,
        "version": VERSION,
...
def get_draft(client: requests.Session, record: str) -> dict:
    ...
    response.raise_for_status()
    value = response.json()
    if str(value.get("id")) != str(record):
...
def prepare() -> dict:
    ...
    if STATE_PATH.is_file():
        ...
        if response.status_code == 409:
            raise RuntimeError("Zenodo reports an unbound new-version draft; refusing duplicate")
        response.raise_for_status()
...
def upload() -> dict:
    ...
    order = {
        "D90-BECKER-01-dualitas-lagrange-slater-kkt-id.pdf": 0,
        "D90-BECKER-01-dualitas-lagrange-slater-kkt-id.html": 1,
        "ADVANCED_OPTIMIZATION_CONVEX_ANALYSIS_ID_BECKER_01_2026.08.25.zip": 2,
        "backend-records-2026.08.25-becker-01.jsonl": 3,
        "backend-records-2026.08.25-becker-01.csv": 4,
        "backend-schema.json": 5,
        RIGHTS_PATH.name: 6,
        MANIFEST_PATH.name: 7,
        SUMS_PATH.name: 8,
    }
    ...
    validate_metadata(response.json()["metadata"])
    validate_draft()
    return {"result": "pass", "draft_id": record, "addition_count": 9, "file_count": 99}
...
def validate_draft() -> dict:
    ...
    if metadata_payload().get("files", {}).get("default_preview") != PRIMARY_PDF:
        raise RuntimeError("local publication payload lacks the Becker-01 PDF preview")
    entries = remote_entries(client, record)
    ...
    if file_state.get("default_preview") != PRIMARY_PDF:
        raise RuntimeError("draft file state lacks the exact Becker-01 preview request")
...
def public_default_preview(client: requests.Session, record: str) -> str:
    ...
    if preview != PRIMARY_PDF:
        raise RuntimeError("public file-state does not select the Becker-01 PDF")
    return preview
...
def repair_default_preview(record: dict) -> dict:
    ...
    if (
        ...
        raise RuntimeError("refusing preview repair outside the published Becker-01 lineage")
    expected = expected_inventory()
    ...
    if file_state.get("default_preview") != PRIMARY_PDF:
        raise RuntimeError("same-record repair did not bind the Becker-01 preview request")
    try:
    ...
    write_json(
        PREVIEW_REPAIR_PATH,
        {
            "schema": "o015-zenodo-becker-01-default-preview-repair-v1",
...
def readback(wait: bool = False, repair_preview: bool = False) -> dict:
    ...
    for item in verified:
        item["disposition"] = "inherited_unchanged" if item["filename"] in inherited_names else "becker_01_addition"
    serialized = json.dumps(record["metadata"], ensure_ascii=False)
    receipt = {
        "schema": "o015-zenodo-becker-01-public-readback-v1",
        ...
        "addition_file_count": sum(item["disposition"] == "becker_01_addition" for item in verified),
...
def main() -> None:
    ...
    parser.add_argument("action", choices=("build", "prepare", "upload", "validate", "publish", "readback", "release"))
