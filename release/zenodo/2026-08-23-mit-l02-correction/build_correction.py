#!/usr/bin/env python3
"""Build the deterministic additive MIT-L02 Zenodo checkpoint."""
...
ROOT = HERE.parents[2]
PREVIOUS_READBACK = HERE.parent / "2026-08-23-mit-l02" / "zenodo-public-readback-mit-l02.json"
STATE_PATH = HERE / "zenodo-draft-mit-l02-correction.json"
FIXED_ZIP_TIME = (2026, 8, 23, 0, 0, 0)

PARENT_RECORD_ID = "22070915"
PARENT_RECORD_DOI = "10.5281/zenodo.22070915"
CONCEPT_ID = "22059741"
CONCEPT_DOI = "10.5281/zenodo.22059741"
VERSION = "checkpoint-2026.08.23-mit-l02-correction"
BUNDLE_NAME = "ADVANCED_OPTIMIZATION_CONVEX_ANALYSIS_ID_SOURCE_BACKEND_2026.08.23_MIT_L02_DELTA.zip"
MANIFEST_NAME = "release-manifest-mit-l02.json"
SUMS_NAME = "SHA256SUMS-mit-l02"
EXPECTED_INHERITED_COUNT = 32
EXPECTED_ADDITION_COUNT = 3
...
RELEASE_DOCS = ["README_MIT_L02_CORRECTION.md", "README_RELEASE_MIT_L02_CORRECTION.md", "RIGHTS_MIT_L02_CORRECTION.md"]
...
def build_bundle() -> tuple[Path, dict[str, object]]:
    ...
    inner = {"schema": "o015-mit-l02-delta-bundle-v1", "version": VERSION, "scope": "MIT 6.253 complete-notes pages 6-13 plus compact source, rights, QA, and backend closure", "complete_corpus": False, "next_boundary": "MIT complete-notes page 14, Modern View of Convex Optimization", "athena_figure_bytes": 0, "entries": entries}
    manifest_bytes = (json.dumps(inner, ensure_ascii=False, indent=2, sort_keys=True) + "\n").encode("utf-8")
    destination = HERE / BUNDLE_NAME
    with tempfile.NamedTemporaryFile(prefix=".mit-l02-delta-", suffix=".zip", dir=HERE, delete=False) as handle:
...
def addition_material_paths() -> list[Path]:
    # The parent already carries the reader and release notes. The correction
    # replaces only the delta bundle; manifest and checksum index are generated
    # separately under their inherited filenames.
    paths = [HERE / BUNDLE_NAME]
    if len(paths) != 1:
        raise RuntimeError("expected one correction material file")
    return paths
...
def addition_paths() -> list[Path]:
    ...
    if len(paths) != EXPECTED_ADDITION_COUNT or len({path.name for path in paths}) != EXPECTED_ADDITION_COUNT:
        raise RuntimeError("expected three correction files")
    missing = [str(path) for path in paths if not path.is_file()]
    if missing:
        raise FileNotFoundError("missing correction release files:\n" + "\n".join(missing))
    return paths
...
def build_release_metadata(bundle_verification: dict[str, object]) -> None:
    ...
    manifest = {"schema": "o015-zenodo-correction-checkpoint-v1", "version": VERSION, "publication_date": "2026-08-23", "title": "Optimisasi Lanjut dan Analisis Konveks - Edisi Bahasa Indonesia", "complete_corpus": False, "correction_of_record_id": "22070915", "correction_of_record_doi": "10.5281/zenodo.22070915", "parent_record_id": PARENT_RECORD_ID, "parent_record_doi": PARENT_RECORD_DOI, "zenodo_concept_id": CONCEPT_ID, "zenodo_concept_doi": CONCEPT_DOI, "zenodo_record_id": record_id, "zenodo_record_doi": record_doi, "release_file_count": EXPECTED_INHERITED_COUNT, "inherited_file_count": EXPECTED_INHERITED_COUNT, "addition_file_count": EXPECTED_ADDITION_COUNT, "replacement_filenames": [BUNDLE_NAME, MANIFEST_NAME, SUMS_NAME], "addition_material_files": material, "generated_addition_files": [MANIFEST_NAME, SUMS_NAME], "delta_bundle_verification": bundle_verification, "admitted_boundary": "MIT 6.253 complete-notes PDF pages 6-13, Duality through Exceptional Behavior", "next_boundary": "MIT 6.253 complete-notes PDF page 14, Modern View of Convex Optimization", "rights_notice": "Component-specific: new MIT-derived reader CC BY-NC-SA 4.0; inherited components retain their own rights.", "model_provenance": "OpenAI Codex gpt-5.6-sol, Ultra"}
    ...
    values.pop(SUMS_NAME, None)
    if len(values) != EXPECTED_INHERITED_COUNT - 1:
...
def validate_local_release(require_draft_binding: bool = False) -> dict[str, object]:
    ...
    if manifest.get("release_file_count") != 32 or manifest.get("addition_file_count") != 3:
    ...
    expected_names = set(inherited_inventory()) - {SUMS_NAME}
    expected_names.update(path.name for path in addition_paths() if path.name != SUMS_NAME)
    if set(checksum_map) != expected_names:
        raise RuntimeError("checksum inventory differs")
    for name, expected in checksum_map.items():
        if name in {BUNDLE_NAME, MANIFEST_NAME}:
            actual = file_digest(HERE / name)
            if actual != expected:
                raise RuntimeError(f"checksum value mismatch: {name}")
    return {"result": "pass", "release_files": 32, "inherited_files": 32, "replacement_files": 3, "delta_bundle": identity(HERE / BUNDLE_NAME), "delta_bundle_verification": bundle_result, "manifest": identity(HERE / MANIFEST_NAME), "checksums": identity(HERE / SUMS_NAME)}
...
def local_inventory() -> dict[str, dict[str, object]]:
    ...
    if len(result) != EXPECTED_INHERITED_COUNT:
