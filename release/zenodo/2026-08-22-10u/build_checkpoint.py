#!/usr/bin/env python3
"""Build the deterministic ten-unit Zenodo checkpoint bundle and manifests."""
...
SOURCE_FILES = [
    ...
    "D90-PENN-05-metode-newton-dan-koreksi-id.tex",
...
PENN_FIGURES = [
    ...
    "NewtonsMethod.pdf",
    "DoublePeak.pdf",
    "GaussModifiedNewtonsMethod.pdf",
    "ModifiedNewton.pdf",
]
...
QA_ALLOWED_NAMES = {
    ...
    "extend_backend_penn_ch05.py",
    "PENN_CH05_INDEPENDENT_REREVIEW.md",
    "PENN_CH05_PROPOSED_LEDGER.jsonl",
    "PENN_CH05_SOLVER_RESULTS.json",
    "PENN_CH05_STRUCTURE_REPORT.json",
    "PENN_CH05_VISUAL_QA.json",
    "validate_acceleration_unit.py",
    "validate_backend_penn_ch03.py",
    "validate_backend_penn_ch04.py",
    "validate_backend_penn_ch05.py",
    "validate_duality_unit.py",
    "validate_optimal_transport_unit.py",
    "validate_penn_ch03_unit.py",
    "validate_penn_ch04_math.py",
    "validate_penn_ch05_math.py",
...
FORBIDDEN_QA_NAMES = {
    "PENN_CH05_SOURCE_AUDIT_DRAFT.md",
    "PENN_CH05_WORKLOG.md",
}
...
def selected_paths() -> list[Path]:
    ...
    paths.extend(ROOT / "source" / "id-ID" / "figures" / item for item in HABRING_FIGURES)
    paths.extend(ROOT / "source" / "id-ID" / "Figures" / item for item in PENN_FIGURES)
    ...
    for path in sorted(path for path in (ROOT / "qa").iterdir() if path.is_file()):
        if path.name in FORBIDDEN_QA_NAMES:
            continue
    ...
    leaked = [
        ...
        for path in ordered
        if any(token in path.relative_to(ROOT).as_posix().lower() for token in UNADMITTED_PENN_TOKENS)
        or path.name in FORBIDDEN_QA_NAMES
        or path.suffix.lower() == ".mpl"
    ]
    if leaked:
        raise RuntimeError("Forbidden or unadmitted input leaked into bundle:\n" + "\n".join(leaked))
    if len(ordered) != EXPECTED_PAYLOADS:
        raise RuntimeError(f"Expected {EXPECTED_PAYLOADS} manifest payloads, got {len(ordered)}")
    return ordered
...
def build_bundle() -> tuple[Path, list[dict[str, object]]]:
    ...
    internal_manifest = {
        ...
        "checkpoint": "ten admitted optional-companion units: Habring Chapters 3-9; Penn Chapters 3-5",
        "complete_corpus": False,
        "entries": inventory,
    }
    manifest_bytes = (json.dumps(internal_manifest, ensure_ascii=False, indent=2, sort_keys=True) + "\n").encode("utf-8")
    ...
    with zipfile.ZipFile(bundle, "r") as archive:
        ...
        for item in parsed["entries"]:
            ...
            if len(data) != item["bytes"] or hashlib.sha256(data).hexdigest() != item["sha256"]:
                raise RuntimeError(f"ZIP inner-manifest mismatch: {item['path']}")
    return bundle, inventory


def draft_identity() -> tuple[str | None, str | None]:
    state = HERE / "zenodo-draft.json"
...
def main() -> None:
    ...
    upload_paths.extend([bundle, HERE / "README_RELEASE.md", HERE / "README.md", HERE / "RIGHTS.md"])
    ...
    release_manifest = {
        ...
        "complete_corpus": False,
        "admitted_units": [
            "Habring Chapter 3", "Habring Chapter 4", "Habring Chapter 5",
            "Habring Chapter 6", "Habring Chapter 7", "Habring Chapter 8",
            "Habring Chapter 9", "Penn Chapter 3", "Penn Chapter 4", "Penn Chapter 5",
        ],
        "uploads": [
    ...
    sums_path.write_text("".join(f"{sha256(path)}  {path.name}\n" for path in checksum_paths), encoding="ascii", newline="\n")
    print(json.dumps({
