#!/usr/bin/env python3
"""Run the two-build local MIT-L10 release gate and write its receipt.
...
RECEIPT_PATH = HERE / "local-preparation-mit-l10.json"
...
def main() -> None:
    ...
    if not build.INPUT_LOCK_PATH.is_file() or build.INPUT_LOCK_PATH.read_bytes() != expected_lock:
        raise RuntimeError("frozen L10 input lock is absent or stale; regenerate it explicitly first")
    ...
    receipt = {
        "schema": "o015-zenodo-mit-l10-local-preparation-v1",
        ...
        "final_backend": {
            ...
            "validation": identity(build.ROOT / "qa/MIT_L10_BACKEND_VALIDATION.json"),
            "correction_snapshot": identity(build.ROOT / "00_control/MIT_L10_CORRECTION_SNAPSHOT.jsonl"),
        },
        "prepublication_control_bindings": first["lane_closure"]["control_bindings"],
        "release_builder": identity(HERE / "build_l10.py"),
        "input_lock_generator": identity(HERE / "generate_input_lock_l10.py"),
        ...
        "publisher": identity(HERE / "publish_l10.py"),
        "network_operations": 0,
        "credential_reads": 0,
        "git_operations": 0,
        "publication_operations": 0,
        "draft_state_file_exists": build.STATE_PATH.is_file(),
        "public_readback_file_exists": (HERE / "zenodo-public-readback-mit-l10.json").is_file(),
        "note": "Local deterministic preservation package only. A future publisher must create one new version from record 22076259 in concept 10.5281/zenodo.22059741, inherit all 82 parent files byte-for-byte, bind the manifest to that record, publish, and anonymously read back all 90 public files.",
    }
